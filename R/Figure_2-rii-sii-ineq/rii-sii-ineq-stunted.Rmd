---
title: "Measuring inequalities: RII and SII - stunted"
...
# Load the data
df_analysis_stunted <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
...
ineq <- df_analysis_stunted %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_stunted) {
    study_name <- unique(df_analysis_stunted$study)
    results <- list()
    
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_stunted %>% filter(arms_maternal == "Control")
      ...
      # Intervention group
      df_interv <- df_analysis_stunted %>% filter(arms_maternal == "SQ-LNS")
...
prevalence_values_stunted <- df_analysis_stunted %>%
...
overall_prevalence_stunted <- df_analysis_stunted %>%
  group_by(study) %>%
  summarise(
...
RII_control_stunted <- c(exp(meta1$TE.random), exp(meta1$upper.random), exp(meta1$lower.random), exp(meta1$seTE.random), meta1$Q, meta1$df.Q, meta1$I2, meta1$tau2)
RII_control_df_stunted <- as.data.frame(RII_control_stunted)
RII_control_df_stunted$meta <- c("RE.meta", "RE.meta.upper", 
                                 "RE.meta.lower", "RE.se",
                              "RE.Q", "RE.df", "RE.I2", "RE.tau2")

RII_control_df_wide_stunted <- RII_control_df_stunted %>%
  pivot_wider(names_from = meta, values_from = RII_control_stunted) 
RII_control_df_wide_stunted$arm <- "Control"
```
...
RII_intervention_stunted <- c(exp(meta2$TE.random), exp(meta2$upper.random), exp(meta2$lower.random), exp(meta2$seTE.random), meta2$Q, meta2$df.Q, meta2$I2, meta2$tau2)
RII_intervention_df_stunted <- as.data.frame(RII_intervention_stunted)
RII_intervention_df_stunted$meta <- c("RE.meta", "RE.meta.upper", 
                                      "RE.meta.lower", "RE.se",
                                   "RE.Q", "RE.df", "RE.I2", "RE.tau2")

RII_intervention_df_wide_stunted <- RII_intervention_df_stunted %>%
  pivot_wider(names_from = meta, values_from = RII_intervention_stunted)
RII_intervention_df_wide_stunted$arm <- "SQ-LNS"
```
...
SII_control_stunted <- c(meta3$TE.random, meta3$upper.random, meta3$lower.random, meta3$seTE.random, meta3$Q, meta3$df.Q, meta3$I2, meta3$tau2)
SII_control_df_stunted <- as.data.frame(SII_control_stunted)
SII_control_df_stunted$meta <- c("RE.meta", "RE.meta.upper", 
                                 "RE.meta.lower", "RE.se",
                               "RE.Q", "RE.df", "RE.I2", "RE.tau2")

SII_control_df_wide_stunted <- SII_control_df_stunted %>%
  pivot_wider(names_from = meta, values_from = SII_control_stunted)
SII_control_df_wide_stunted$arm <- "Control"
```
...
SII_intervention_stunted <- c(meta4$TE.random, meta4$upper.random, meta4$lower.random, meta4$seTE.random, meta4$Q, meta4$df.Q, meta4$I2, meta4$tau2)
SII_intervention_df_stunted <- as.data.frame(SII_intervention_stunted)
SII_intervention_df_stunted$meta <- c("RE.meta", "RE.meta.upper", "RE.meta.lower",
                                      "RE.se",
                                   "RE.Q", "RE.df", "RE.I2", "RE.tau2")

SII_intervention_df_wide_stunted <- SII_intervention_df_stunted %>%
  pivot_wider(names_from = meta, values_from = SII_intervention_stunted)
SII_intervention_df_wide_stunted$arm <- "SQ-LNS"
```
...
prevalence_sqlns_stunted <- c(meta5$TE.random, meta5$upper.random, meta5$lower.random, meta5$seTE.random, meta5$Q, meta5$df.Q, meta5$I2, meta5$tau2)
prevalence_sqlns_df_stunted <- as.data.frame(prevalence_sqlns_stunted)
prevalence_sqlns_df_stunted$meta <- c("RE.meta", "RE.meta.upper", 
                                      "RE.meta.lower", "RE.se",
                              "RE.Q", "RE.df", "RE.I2", "RE.tau2")

prevalence_sqlns_df_wide_stunted <- prevalence_sqlns_df_stunted %>%
  pivot_wider(names_from = meta, values_from = prevalence_sqlns_stunted)
prevalence_sqlns_df_wide_stunted$arm <- "SQ-LNS"
```
...
prevalence_control_stunted <- c(meta6$TE.random, meta6$upper.random, meta6$lower.random, meta6$seTE.random, meta6$Q, meta6$df.Q, meta6$I2, meta6$tau2)
prevalence_control_df_stunted <- as.data.frame(prevalence_control_stunted)
prevalence_control_df_stunted$meta <- c("RE.meta", "RE.meta.upper", 
                                        "RE.meta.lower", "RE.se",
                              "RE.Q", "RE.df", "RE.I2", "RE.tau2")

prevalence_control_df_wide_stunted <- prevalence_control_df_stunted %>%
  pivot_wider(names_from = meta, values_from = prevalence_control_stunted)
prevalence_control_df_wide_stunted$arm <- "Control"
```
...
meta_results_rii_stunted <- rbind(RII_control_df_wide_stunted, RII_intervention_df_wide_stunted)
meta_results_prevalence_stunted <- rbind(prevalence_control_df_wide_stunted, prevalence_sqlns_df_wide_stunted)
colnames(meta_results_prevalence_stunted) <- c("RE.prev", "RE.prev.upper", 
                                       "RE.prev.lower", "RE.prev.se",
                                       "RE.prev.Q", "RE.prev.df", 
                                       "RE.prev.I2", "RE.prev.tau2", "arm.prev")
meta_results_rii_prev_stunted <- cbind(meta_results_rii_stunted,
                               meta_results_prevalence_stunted)


meta_results_sii_stunted <- rbind(SII_control_df_wide_stunted,
                                  SII_intervention_df_wide_stunted)
meta_results_sii_prev_stunted <- cbind(meta_results_sii_stunted,
                                       meta_results_prevalence_stunted)
...
meta_rii_stunted <- ggplot(aes(x = arm, y = RE.meta), 
                           data = meta_results_rii_stunted) +
  geom_point(aes(color = arm), size = 2.5) +
  geom_errorbar(aes(x = arm, ymin = RE.meta.lower, ymax = RE.meta.upper), width = 0.1, data = meta_results_rii_stunted) +
...
meta_sii_stunted <- ggplot(meta_results_sii_stunted, aes(x = arm, y = RE.meta*100)) +
  geom_point(aes(color = arm), size = 2.5) +
...
meta_plot_stunted <- plot_grid(meta_rii_stunted, meta_sii_stunted, ncol = 1, nrow = 2)
...
#write.csv2(ineq, file = here::here("output", "rii_sii_stunted.csv"), row.names = FALSE)
