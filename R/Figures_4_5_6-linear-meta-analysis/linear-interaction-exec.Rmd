---
title: "Linear regression - interaction between treatment and IWI - executive function score"
...
iwi_values <- seq(min(df_analysis_exec$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_exec %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_exec) {
    study_name <- unique(df_analysis_exec$study)
...
point.wise.DF.plot = point.wise.DF %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled d12_execfun_el')) +
  theme_minimal() +
...
study_list <- unique(df_analysis_exec$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_exec %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_exec$iwi), by = 1, length.out = 71)
