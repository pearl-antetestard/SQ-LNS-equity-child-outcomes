---
title: "Linear regression - interaction between treatment and IWI - wlz"
...
iwi_values <- seq(min(df_analysis_wlz$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_wlz %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wlz) {
    study_name <- unique(df_analysis_wlz$study)
...
study_list <- unique(df_analysis_wlz$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_wlz %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_wlz$iwi), by = 1, length.out = 71)
