---
title: "Linear regression - interaction between treatment and IWI - language score"
...
iwi_values <- seq(min(df_analysis_language$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_language %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_language) {
    study_name <- unique(df_analysis_language$study)
...
study_list <- unique(df_analysis_language$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_language %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_language$iwi), by = 1, length.out = 71)
