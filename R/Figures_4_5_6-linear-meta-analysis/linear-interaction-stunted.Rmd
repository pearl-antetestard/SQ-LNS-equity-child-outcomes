...
inter <- df_analysis_stunted %>%
  ...
  map_dfr(function(df_analysis_stunted) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(stunted_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_stunted, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_stunted$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_stunted %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_stunted) {
    study_name <- unique(df_analysis_stunted$study)
...
study_list <- unique(df_analysis_stunted$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_stunted %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_stunted$iwi), by = 1, length.out = 71)
