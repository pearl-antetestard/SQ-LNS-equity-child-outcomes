---
title: "Measuring inequalities: RII and SII - wlz"
...
# Load the data
df_analysis_wlz <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
...
RII > 1 or SII > 0: wlz_el is higher among the poor than the rich; 
RII = 1 or SII = 0: no inequality

```{r RII and SII}

ineq <- df_analysis_wlz %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wlz) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_wlz %>% filter(arms_maternal == "Control")
      #min_wlz_el <- min(df_control$wlz_el)

      #df_control$Y_shifted <- df_control$wlz_el + abs(min_wlz_el) + 1

      #glm_c <- glm(Y_shifted ~ iwi_relative_rank, family = gaussian(link = "log"), 
       #            data = df_control)
      lm_c <- lm_robust(wlz_el ~ iwi_relative_rank, data = df_control) 
      ...
      RII_lm_c <-  coef(lm_c)[2] / mean(df_control$wlz_el)
      #log_RII_lm_c <- coef(glm_c)[2]
      #se_log_rii <- summary(glm_c)$coefficients[2, "Std. Error"]
      #deltamethod(~exp(x1),log_RII_lm_c,se_log_rii^2) 
      ...
      # Intervention group
      df_interv <- df_analysis_wlz %>% filter(arms_maternal == "SQ-LNS")
      lm_i <- lm_robust(wlz_el ~ iwi_relative_rank, data = df_interv) 
      ...
      # Control group
      df_control <- df_analysis_wlz %>% filter(arms_maternal == "Control")
      lm_c <- lm_robust(wlz_el ~ iwi_relative_rank, clusters = cluster, data = df_control) 
      ...
      # Intervention group
      df_interv <- df_analysis_wlz %>% filter(arms_maternal == "SQ-LNS")
      lm_i <- lm_robust(wlz_el ~ iwi_relative_rank, clusters = cluster, data = df_interv) 
...
# Calculate the prevalence of wlz_el by study
prevalence_values_wlz <- df_analysis_wlz %>%
  group_by(study, arms_maternal) %>%
  summarise(
    prevalence_wlz_el = mean(wlz_el == 1, na.rm = TRUE),
    sd_wlz_el = sd(wlz_el == 1, na.rm = TRUE)
  ) 

# Calculate the overall prevalence of wlz_el by study
overall_prevalence_wlz <- df_analysis_wlz %>%
  group_by(study) %>%
  summarise(
    prevalence_wlz_el = mean(wlz_el == 1, na.rm = TRUE),  # Calculate the prevalence
    sd_wlz_el = sd(wlz_el == 1, na.rm = TRUE)
  )
...
df_sqlns <- subset(prevalence_values_wlz, (arms_maternal %in% c("SQ-LNS"))) 
colnames(df_sqlns) <- c("study", "arms_maternal", "prevalence_wlz_el_sqlns",
                        "sd_wlz_el_sqlns")
df_control <- subset(prevalence_values_wlz, (arms_maternal %in% c("Control")))
colnames(df_control) <- c("study", "arms_maternal", "prevalence_wlz_el_control",
                        "sd_wlz_el_control")
df <- left_join(ineq, df_sqlns, by = "study") 
df <- left_join(df, df_control, by = "study")
df <- left_join(df, overall_prevalence_wlz, by = "study")
...
### Prevalence SQ-LNS
meta5 <- metagen(TE = df$prevalence_wlz_el_sqlns, seTE = df$sd_wlz_el_sqlns, 
...
### Prevalence control
meta6 <- metagen(TE = df$prevalence_wlz_el_control, 
                 seTE = df$sd_wlz_el_control, studlab = df$study,
...
meta_rii_wlz <- ggplot(aes(x = arm, y = RE.meta), data = meta_results_rii) +
  ...
  labs(x = "wlz", y = "Pooled RII (linear)") +
...
meta_sii_wlz <- ggplot(meta_results_sii, aes(x = arm, y = RE.meta*100)) +
   geom_point(aes(color = arm), size = 2.5) +
  geom_errorbar(aes(ymin = RE.meta.lower*100, ymax = RE.meta.upper*100), 
                width = 0.1) +
  labs(x = "wlz", y = "Pooled SII %-point") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  theme_minimal() +
   scale_color_manual(values = c("Control" = "darkgrey", "SQ-LNS" = "darkblue")) +
  theme(legend.position = "none") 
...
meta_plot_wlz <- plot_grid(meta_rii_wlz, meta_sii_wlz, ncol = 1, nrow = 2)
...
#write.csv2(ineq, file = here::here("output", "rii_sii_wlz.csv"), row.names = FALSE)
