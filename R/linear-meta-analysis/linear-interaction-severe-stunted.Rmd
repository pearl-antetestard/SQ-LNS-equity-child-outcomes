---
title: "Linear regression - interaction between treatment and IWI - severe stunting"
...
inter <- df_analysis_severe_stunted %>%
  ...
  map_dfr(function(df_analysis_severe_stunted) {
    ...
    if (study_name %in% c("DOSE")) {
      ...
      model <- lm_robust(severe_stunted ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_severe_stunted, clusters = cluster)
...
laz_eff_df_wide$RE.meta  =        expit(laz_eff_df_wide$RE.meta)
laz_eff_df_wide$RE.meta.upper  =  expit(laz_eff_df_wide$RE.meta.upper)
laz_eff_df_wide$RE.meta.lower  =  expit(laz_eff_df_wide$RE.meta.lower)
...
iwi_values <- seq(min(df_analysis_severe_stunted$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_severe_stunted %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_severe_stunted) {
    study_name <- unique(df_analysis_severe_stunted$study)
...
## Backtransform predicted outcomes and their corresponding confidence intervals
point.wise.DF$RE.meta  =        expit(point.wise.DF$RE.meta)
point.wise.DF$RE.meta.upper  =  expit(point.wise.DF$RE.meta.upper)
point.wise.DF$RE.meta.lower  =  expit(point.wise.DF$RE.meta.lower)
...
study_list <- unique(df_analysis_severe_stunted$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_severe_stunted %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_severe_stunted$iwi), by = 1, length.out = 71)
...
plot_fit %>%
  mutate(Treatment = as.factor(arms_maternal)) %>%
  ggplot(aes(x = iwi, y = expit(predicted), fill = Treatment, linetype = Treatment)) +
  geom_line(size=0.5, aes(colour = Treatment)) +
  scale_color_manual(values = c('darkgrey','darkblue')) + 
  scale_fill_manual(values = c('darkgrey','darkblue')) + 
  geom_ribbon(aes(ymin = expit(ci_lower), ymax = expit(ci_upper), fill = arms_maternal), alpha = 0.2) +
  labs(color='Treatment', fill='Treatment', linetype='Treatment') +
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled probability of severe stunting')) +
  theme_minimal() +
