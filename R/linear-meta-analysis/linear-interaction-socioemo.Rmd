---
title: "Linear regression - interaction between treatment and IWI - socioemotional score"
...
iwi_values <- seq(min(df_analysis_socioemo$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_socioemo %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_socioemo) {
    study_name <- unique(df_analysis_socioemo$study)
...
point.wise.DF.plot = point.wise.DF %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled d5_socioemo_el')) +
  theme_minimal() +
...
study_list <- unique(df_analysis_socioemo$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_socioemo %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_socioemo$iwi), by = 1, length.out = 71)
