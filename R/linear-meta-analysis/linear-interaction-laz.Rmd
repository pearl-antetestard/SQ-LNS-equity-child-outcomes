---
title: "Linear regression - interaction between treatment and IWI - laz"
...
df_analysis_laz <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
inter <- df_analysis_laz %>%
  ...
  map_dfr(function(df_analysis_laz) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(laz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_laz)
      ...
      model <- lm_robust(laz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_laz, clusters = cluster)
...
ggplot(inter, aes(x = study, y = inter_treat_iwi)) +
  ...
  labs(x = "Study", y = "laz_el (arms_maternalSQ-LNS:iwi)") +
...
forest_meta <- forest(meta1, 
       layout = "JAMA", 
       leftlabs = c("Study", "laz_el (iwi:arms_maternalSQ-LNS) [95% CI]"),
...
## The pooled interaction term iwi:arms_maternalSQ-LNS is not statistically significant at the 0.05 level, indicating that the effect of iwi on laz_el does not significantly differ between the SQ-LNS and control groups. Although the direction of the pooled slope is positive, indicating that in the SQ-LNS group (compared to the control group) is associated with an increase of approximately 0.0006 in laz_el, holding iwi constant.
...
pooled_slope_plot <- ggplot(aes(x = term, y = RE.meta), data = laz_eff_df_wide) +
  ...
  labs(x = "Random-effect meta-analysis", y = "Pooled laz_el (iwi:arms_maternalSQ-LNS)") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  theme_minimal()  +
...
## The interaction p-values are from the linear regression models with the interaction term (iwi:arms_maternalSQ-LNS). The p-values indicate whether the effect of IWI on laz_el differs significantly between the SQ-LNS and control groups.

```{r interaction pred, echo=TRUE, warning=FALSE}

# Define a range of iwi values for prediction
iwi_values <- seq(min(df_analysis_laz$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_laz %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_laz) {
    study_name <- unique(df_analysis_laz$study)
...
study_list <- unique(df_analysis_laz$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_laz %>% filter(study == !!study)
  
  if (study %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
    model_study <- lm_robust(laz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                             data = df_study)
  } else {
    model_study <- lm_robust(laz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_laz$iwi), by = 1, length.out = 71)
...
## 3rd method
This method uses function risk.diff.creator to compute the absolute differences between two groups based on predicted outcomes and their confidence intervals like the ones used with the GAMs and the first method for linear model. Constructs confidence intervals for the difference (diff.lower, diff.upper) by propagating uncertainty from the confidence intervals of both groups. The method uses the Pythagorean-like rule to account for the variances of each group.(p1-l1) represents how far the predicted outcome of group 1 is from its lower bound. (u2-p2) represents how far the predicted outcome of group 2 is from its upper bound.


```{r alt2, echo=TRUE, warning=FALSE}


absolute_diff = risk.diff.creator(dataframe = plot_fit,
                                  treatment = "arms_maternal", outcome = NULL,
                                  matching.variables = c("iwi"),
                                  predicted.outcome = "predicted", 
                                  predicted.CI = c("ci_lower","ci_upper"))


absolute_diff_plot <- absolute_diff %>%
  ggplot(aes(x = iwi,y=fit.diff)) + geom_line(size=0.5)+
  ...
  theme(axis.title.y = element_text(size = 18),
        ...
        axis.text.y = element_text(face="bold",  size=12),
        axis.text.x = element_text(face="bold",  size=12)) + #ylim(c(-0.15,0.4)) +
      annotate("text", x = min(absolute_diff$iwi) * 0.8, 
