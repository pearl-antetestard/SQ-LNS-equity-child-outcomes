---
title: "Linear regression - interaction between treatment and IWI - stunted"
...
inter <- df_analysis_wasted %>%
  ...
  map_dfr(function(df_analysis_wasted) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(wasted_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_wasted, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_wasted$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_wasted %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wasted) {
    study_name <- unique(df_analysis_wasted$study)
...
study_list <- unique(df_analysis_wasted$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_wasted %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_wasted$iwi), by = 1, length.out = 71)
