---
title: "Linear regression - interaction between treatment and IWI - anemia"
...
inter <- df_analysis_anemia %>%
  ...
  map_dfr(function(df_analysis_anemia) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(b2_anemia_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_anemia, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_anemia$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions <- df_analysis_anemia %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_anemia) {
    study_name <- unique(df_analysis_anemia$study)
...
study_list <- unique(df_analysis_anemia$study)
results <- lapply(study_list, function(study) {
  df_study <- df_analysis_anemia %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_anemia$iwi), by = 1, length.out = 71)
