---
title: "Absolute treatment effect by Pointwise meta-analysis - executive function score"
...
df_analysis_exec <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
SS.Comb_exec <- df_analysis_exec %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_exec) {
    study_name <- unique(df_analysis_exec$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(d12_execfun_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_exec,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(d12_execfun_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_exec <- df_analysis_exec %>%
...
predictions.SS.Comb_exec <- new.data_exec %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_exec, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_exec)
...
plot_study_exec <- predictions.SS.Comb_exec%>%
...
point.wise.DF.SS.Comb_exec =  pointwise.ma(predictions.SS.Comb_exec,
...
point.wise.DF.SS.Comb_exec$RE.meta  =        point.wise.DF.SS.Comb_exec$RE.meta
point.wise.DF.SS.Comb_exec$RE.meta.upper  =  point.wise.DF.SS.Comb_exec$RE.meta.upper
point.wise.DF.SS.Comb_exec$RE.meta.lower  =  point.wise.DF.SS.Comb_exec$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_exec = point.wise.DF.SS.Comb_exec%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled executive function score')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_exec$iwi),  max(point.wise.DF.SS.Comb_exec$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_exec)
...
predictions.SS.Comb_exec=predictions.SS.Comb_exec%>%
...
absolute_diff_SS.Comb_exec = risk.diff.creator(dataframe = predictions.SS.Comb_exec,
...
absolute_diff_SS.Comb_exec=  absolute_diff_SS.Comb_exec%>%
...
point.wise.absolute_diff_SS.Comb_exec  =  pointwise.ma(data = absolute_diff_SS.Comb_exec,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_exec =  point.wise.absolute_diff_SS.Comb_exec%>%
...
point.wise.DF.SS.Comb.diff.plot_exec=point.wise.absolute_diff_SS.Comb_exec%>%
...
print(point.wise.DF.SS.Comb.diff.plot_exec)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_exec=point.wise.absolute_diff_SS.Comb_exec%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_exec)

point.wise.pvalQ.plot_exec=point.wise.absolute_diff_SS.Comb_exec%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_exec)


plot_composite_hetero_exec <- plot_grid(point.wise.Q.plot_exec, point.wise.pvalQ.plot_exec,
                               nrow=2, align = "h") 
plot_composite_hetero_exec
...
plot_comp_effect_exec <- plot_grid(point.wise.DF.SS.Comb.plot_exec, point.wise.DF.SS.Comb.diff.plot_exec, ncol = 1, labels = c("B", "C"))


plot_composite_exec <- plot_grid(plot_study_exec, plot_comp_effect_exec,
                               ncol=2, labels = c("A", "", "")) 
