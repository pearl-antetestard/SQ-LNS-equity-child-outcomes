---
title: "Absolute treatment effect by Pointwise meta-analysis - hemoglobin concentration"
...
df_analysis_hgb <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
SS.Comb_hgb <- df_analysis_hgb %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_hgb) {
    study_name <- unique(df_analysis_hgb$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(b1_hb_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_hgb,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(b1_hb_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
predictions.SS.Comb_hgb <- new.data_hgb %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_hgb, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_hgb)
...
plot_study_hgb <- predictions.SS.Comb_hgb%>%
...
point.wise.DF.SS.Comb_hgb =  pointwise.ma(predictions.SS.Comb_hgb,
...
point.wise.DF.SS.Comb_hgb$RE.meta  =        point.wise.DF.SS.Comb_hgb$RE.meta
point.wise.DF.SS.Comb_hgb$RE.meta.upper  =  point.wise.DF.SS.Comb_hgb$RE.meta.upper
point.wise.DF.SS.Comb_hgb$RE.meta.lower  =  point.wise.DF.SS.Comb_hgb$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_hgb = point.wise.DF.SS.Comb_hgb%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled hemoglobin concentration')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_hgb$iwi),  max(point.wise.DF.SS.Comb_hgb$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_hgb)
...
predictions.SS.Comb_hgb=predictions.SS.Comb_hgb%>%
...
absolute_diff_SS.Comb_hgb = risk.diff.creator(dataframe = predictions.SS.Comb_hgb,
...
absolute_diff_SS.Comb_hgb=  absolute_diff_SS.Comb_hgb%>%
...
point.wise.absolute_diff_SS.Comb_hgb  =  pointwise.ma(data = absolute_diff_SS.Comb_hgb,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_hgb =  point.wise.absolute_diff_SS.Comb_hgb%>%
...
point.wise.DF.SS.Comb.diff.plot_hgb=point.wise.absolute_diff_SS.Comb_hgb%>%
...
print(point.wise.DF.SS.Comb.diff.plot_hgb)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_hgb=point.wise.absolute_diff_SS.Comb_hgb%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_hgb)

point.wise.pvalQ.plot_hgb=point.wise.absolute_diff_SS.Comb_hgb%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_hgb)


plot_composite_hetero_hgb <- plot_grid(point.wise.Q.plot_hgb, point.wise.pvalQ.plot_hgb,
                               nrow=2, align = "h") 
plot_composite_hetero_hgb
...
plot_comp_effect_hgb <- plot_grid(point.wise.DF.SS.Comb.plot_hgb, point.wise.DF.SS.Comb.diff.plot_hgb, ncol = 1, labels = c("B", "C"))


plot_composite_hgb <- plot_grid(plot_study_hgb, plot_comp_effect_hgb,
                               ncol=2, labels = c("A", "", "")) 
