---
title: "Absolute treatment effect by Pointwise meta-analysis - wlz"
...
df_analysis_wlz <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
SS.Comb_wlz <- df_analysis_wlz %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wlz) {
    study_name <- unique(df_analysis_wlz$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(wlz_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_wlz,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(wlz_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
predictions.SS.Comb_wlz <- new.data_wlz %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_wlz, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_wlz)
...
plot_study_wlz <- predictions.SS.Comb_wlz %>%
...
point.wise.DF.SS.Comb_wlz =  pointwise.ma(predictions.SS.Comb_wlz,
...
point.wise.DF.SS.Comb_wlz$RE.meta  =        point.wise.DF.SS.Comb_wlz$RE.meta
point.wise.DF.SS.Comb_wlz$RE.meta.upper  =  point.wise.DF.SS.Comb_wlz$RE.meta.upper
point.wise.DF.SS.Comb_wlz$RE.meta.lower  =  point.wise.DF.SS.Comb_wlz$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_wlz = point.wise.DF.SS.Comb_wlz %>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_wlz$iwi),  max(point.wise.DF.SS.Comb_wlz$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_wlz)
...
predictions.SS.Comb_wlz=predictions.SS.Comb_wlz %>%
...
absolute_diff_SS.Comb_wlz = risk.diff.creator(dataframe = predictions.SS.Comb_wlz,
...
absolute_diff_SS.Comb_wlz=  absolute_diff_SS.Comb_wlz%>%
...
point.wise.absolute_diff_SS.Comb_wlz  =  pointwise.ma(data = absolute_diff_SS.Comb_wlz ,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_wlz =  point.wise.absolute_diff_SS.Comb_wlz%>%
...
point.wise.DF.SS.Comb.diff.plot_wlz=point.wise.absolute_diff_SS.Comb_wlz%>%
...
print(point.wise.DF.SS.Comb.diff.plot_wlz)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_wlz=point.wise.absolute_diff_SS.Comb_wlz%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_wlz)

point.wise.pvalQ.plot_wlz=point.wise.absolute_diff_SS.Comb_wlz%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_wlz)


plot_composite_hetero_wlz <- plot_grid(point.wise.Q.plot_wlz, point.wise.pvalQ.plot_wlz,
                               nrow=2, align = "h") 
plot_composite_hetero_wlz
...
plot_comp_effect_wlz <- plot_grid(point.wise.DF.SS.Comb.plot_wlz, point.wise.DF.SS.Comb.diff.plot_wlz, ncol = 1, labels = c("B", "C"))


plot_composite_wlz <- plot_grid(plot_study_wlz, plot_comp_effect_wlz,
                               ncol=2, labels = c("A", "", "")) 
