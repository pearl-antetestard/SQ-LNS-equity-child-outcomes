---
title: "Absolute treatment effect by Pointwise meta-analysis - severe wasted"
...
df_analysis <- readRDS(file = here::here("data", "1-final",
  ...
  dplyr::mutate(arms_maternal = factor(arms_maternal, levels = c(0,1), 
                ...
                severe_wasted = case_when(wlz_el < -3 ~ 1,
                                           wlz_el >= -3 ~ 0),
                severe_wasted = factor(severe_wasted),
         iwi = as.numeric(iwi),
...
table(df_analysis$study, df_analysis$severe_wasted)
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(severe_wasted ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb <- df_analysis %>%
  ...
  map_dfr(function(df_analysis) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(severe_wasted ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis,
...
plot_study <- predictions.SS.Comb%>%
  ...
  ylab("Probability of severe wasting") +
  xlab("IWI") +
...
point.wise.DF.SS.Comb.plot = point.wise.DF.SS.Comb%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled probability of severe wasting')) +
  theme_minimal() +
...
# Save the plot
ggsave(here::here("output", "gam_pooled_pointwise_severe_wasted_arm_maternal.png"),
