---
title: "Absolute treatment effect by Pointwise meta-analysis - gross motor score"
...
SS.Comb_gross <- df_analysis_gross %>%
  split(.$study) %>%
...
new.data_gross <- df_analysis_gross %>%
...
predictions.SS.Comb_gross <- new.data_gross %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_gross, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_gross)
...
plot_study_gross <- predictions.SS.Comb_gross%>%
...
point.wise.DF.SS.Comb_gross =  pointwise.ma(predictions.SS.Comb_gross,
...
point.wise.DF.SS.Comb_gross$RE.meta  =        point.wise.DF.SS.Comb_gross$RE.meta
point.wise.DF.SS.Comb_gross$RE.meta.upper  =  point.wise.DF.SS.Comb_gross$RE.meta.upper
point.wise.DF.SS.Comb_gross$RE.meta.lower  =  point.wise.DF.SS.Comb_gross$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_gross = point.wise.DF.SS.Comb_gross%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled gross motor score')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_gross$iwi),  max(point.wise.DF.SS.Comb_gross$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_gross)
...
predictions.SS.Comb_gross=predictions.SS.Comb_gross%>%
...
absolute_diff_SS.Comb_gross = risk.diff.creator(dataframe = predictions.SS.Comb_gross,
...
absolute_diff_SS.Comb_gross=  absolute_diff_SS.Comb_gross%>%
...
point.wise.absolute_diff_SS.Comb_gross  =  pointwise.ma(data = absolute_diff_SS.Comb_gross,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_gross =  point.wise.absolute_diff_SS.Comb_gross%>%
...
point.wise.DF.SS.Comb.diff.plot_gross=point.wise.absolute_diff_SS.Comb_gross%>%
...
print(point.wise.DF.SS.Comb.diff.plot_gross)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_gross=point.wise.absolute_diff_SS.Comb_gross%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_gross)

point.wise.pvalQ.plot_gross=point.wise.absolute_diff_SS.Comb_gross%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_gross)


plot_composite_hetero_gross <- plot_grid(point.wise.Q.plot_gross, point.wise.pvalQ.plot_gross,
                               nrow=2, align = "h") 
plot_composite_hetero_gross
...
plot_comp_effect_gross <- plot_grid(point.wise.DF.SS.Comb.plot_gross, point.wise.DF.SS.Comb.diff.plot_gross, ncol = 1, labels = c("B", "C"))


plot_composite_gross <- plot_grid(plot_study_gross, plot_comp_effect_gross,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_gross
