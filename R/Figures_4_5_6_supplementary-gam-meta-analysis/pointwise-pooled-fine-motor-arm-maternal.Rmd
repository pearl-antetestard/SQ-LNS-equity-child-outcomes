---
title: "Absolute treatment effect by Pointwise meta-analysis - fine motor score"
...
df_analysis_fine <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
SS.Comb_fine <- df_analysis_fine %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_fine) {
    study_name <- unique(df_analysis_fine$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(d10_finemotor_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_fine,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(d10_finemotor_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_fine <- df_analysis_fine %>%
...
predictions.SS.Comb_fine <- new.data_fine %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_fine, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_fine)
...
plot_study_fine <- predictions.SS.Comb_fine%>%
...
point.wise.DF.SS.Comb_fine =  pointwise.ma(predictions.SS.Comb_fine,
...
point.wise.DF.SS.Comb_fine$RE.meta  =        point.wise.DF.SS.Comb_fine$RE.meta
point.wise.DF.SS.Comb_fine$RE.meta.upper  =  point.wise.DF.SS.Comb_fine$RE.meta.upper
point.wise.DF.SS.Comb_fine$RE.meta.lower  =  point.wise.DF.SS.Comb_fine$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_fine = point.wise.DF.SS.Comb_fine%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled fine motor score')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_fine$iwi),  max(point.wise.DF.SS.Comb_fine$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_fine)
...
predictions.SS.Comb_fine=predictions.SS.Comb_fine%>%
...
absolute_diff_SS.Comb_fine = risk.diff.creator(dataframe = predictions.SS.Comb_fine,
...
absolute_diff_SS.Comb_fine=  absolute_diff_SS.Comb_fine%>%
...
point.wise.absolute_diff_SS.Comb_fine  =  pointwise.ma(data = absolute_diff_SS.Comb_fine,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_fine =  point.wise.absolute_diff_SS.Comb_fine%>%
...
point.wise.DF.SS.Comb.diff.plot_fine=point.wise.absolute_diff_SS.Comb_fine%>%
...
print(point.wise.DF.SS.Comb.diff.plot_fine)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_fine=point.wise.absolute_diff_SS.Comb_fine%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_fine)

point.wise.pvalQ.plot_fine=point.wise.absolute_diff_SS.Comb_fine%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_fine)


plot_composite_hetero_fine <- plot_grid(point.wise.Q.plot_fine, point.wise.pvalQ.plot_fine,
                               nrow=2, align = "h") 
plot_composite_hetero_fine
...
plot_comp_effect_fine <- plot_grid(point.wise.DF.SS.Comb.plot_fine, point.wise.DF.SS.Comb.diff.plot_fine, ncol = 1, labels = c("B", "C"))


plot_composite_fine <- plot_grid(plot_study_fine, plot_comp_effect_fine,
                               ncol=2, labels = c("A", "", "")) 
