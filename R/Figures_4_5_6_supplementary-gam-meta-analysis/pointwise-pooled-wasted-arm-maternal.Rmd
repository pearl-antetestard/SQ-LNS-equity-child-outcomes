...
df_analysis_wasted <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
table(df_analysis_wasted$study, df_analysis_wasted$wasted_el)
...
df_analysis_wasted <- subset(df_analysis_wasted, !(study %in% c("HAITI", "PROMISM")))
...
SS.Comb_wasted <- df_analysis_wasted %>%
...
new.data_wasted <- df_analysis_wasted %>%
...
predictions.SS.Comb_wasted <- new.data_wasted %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_wasted, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_wasted)
...
plot_study_wasted <- predictions.SS.Comb_wasted%>%
...
point.wise.DF.SS.Comb_wasted =  pointwise.ma(predictions.SS.Comb_wasted,
...
point.wise.DF.SS.Comb_wasted$RE.meta  =        expit(point.wise.DF.SS.Comb_wasted$RE.meta)
point.wise.DF.SS.Comb_wasted$RE.meta.upper  =  expit(point.wise.DF.SS.Comb_wasted$RE.meta.upper)
point.wise.DF.SS.Comb_wasted$RE.meta.lower  =  expit(point.wise.DF.SS.Comb_wasted$RE.meta.lower)
...
point.wise.DF.SS.Comb.plot_wasted = point.wise.DF.SS.Comb_wasted%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled probability of wasting')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_wasted$iwi), max(point.wise.DF.SS.Comb_wasted$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_wasted)
...
predictions.SS.Comb_wasted=predictions.SS.Comb_wasted%>%
...
absolute_diff_SS.Comb_wasted = risk.diff.creator(dataframe = predictions.SS.Comb_wasted,
...
absolute_diff_SS.Comb_wasted=  absolute_diff_SS.Comb_wasted%>%
...
point.wise.absolute_diff_SS.Comb_wasted  =  pointwise.ma(data = absolute_diff_SS.Comb_wasted,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_wasted =  point.wise.absolute_diff_SS.Comb_wasted%>%
...
point.wise.DF.SS.Comb.diff.plot_wasted=point.wise.absolute_diff_SS.Comb_wasted%>%
...
print(point.wise.DF.SS.Comb.diff.plot_wasted)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_wasted=point.wise.absolute_diff_SS.Comb_wasted%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_wasted)

point.wise.pvalQ.plot_wasted=point.wise.absolute_diff_SS.Comb_wasted%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_wasted)


plot_composite_hetero_wasted <- plot_grid(point.wise.Q.plot_wasted, point.wise.pvalQ.plot_wasted,
                               nrow=2, align = "h") 
plot_composite_hetero_wasted
...
plot_comp_effect_wasted <- plot_grid(point.wise.DF.SS.Comb.plot_wasted, point.wise.DF.SS.Comb.diff.plot_wasted, ncol = 1, labels = c("B", "C"))


plot_composite_wasted <- plot_grid(plot_study_wasted, plot_comp_effect_wasted,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_wasted
