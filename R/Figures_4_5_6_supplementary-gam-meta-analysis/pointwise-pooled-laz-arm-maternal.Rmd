---
title: "Absolute treatment effect by Pointwise meta-analysis - laz"
...
predictions.SS.Comb_laz <- new.data_laz %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_laz, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_laz)
...
plot_study_laz <- predictions.SS.Comb_laz%>%
...
point.wise.DF.SS.Comb_laz =  pointwise.ma(predictions.SS.Comb_laz,
...
point.wise.DF.SS.Comb_laz$RE.meta  =        point.wise.DF.SS.Comb_laz$RE.meta
point.wise.DF.SS.Comb_laz$RE.meta.upper  =  point.wise.DF.SS.Comb_laz$RE.meta.upper
point.wise.DF.SS.Comb_laz$RE.meta.lower  =  point.wise.DF.SS.Comb_laz$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_laz = point.wise.DF.SS.Comb_laz%>%
...
scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_laz$iwi),   max(point.wise.DF.SS.Comb_laz$iwi), by = 10))




print(point.wise.DF.SS.Comb.plot_laz)
...
predictions.SS.Comb_laz=predictions.SS.Comb_laz%>%
...
absolute_diff_SS.Comb_laz = risk.diff.creator(dataframe = predictions.SS.Comb_laz,
...
absolute_diff_SS.Comb_laz =  absolute_diff_SS.Comb_laz%>%
...
point.wise.absolute_diff_SS.Comb_laz  =  pointwise.ma(data = absolute_diff_SS.Comb_laz,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_laz =  point.wise.absolute_diff_SS.Comb_laz%>%
...
point.wise.DF.SS.Comb.diff.plot_laz=point.wise.absolute_diff_SS.Comb_laz%>%
...
print(point.wise.DF.SS.Comb.diff.plot_laz)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_laz=point.wise.absolute_diff_SS.Comb_laz%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_laz)

point.wise.pvalQ.plot_laz=point.wise.absolute_diff_SS.Comb_laz%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_laz)


plot_composite_hetero_laz <- plot_grid(point.wise.Q.plot_laz, point.wise.pvalQ.plot_laz,
                               nrow=2, align = "h") 
plot_composite_hetero_laz
...
plot_comp_effect_laz <- plot_grid(point.wise.DF.SS.Comb.plot_laz, point.wise.DF.SS.Comb.diff.plot_laz, ncol = 1, labels = c("B", "C"))


plot_composite_laz <- plot_grid(plot_study_laz, plot_comp_effect_laz,
                               ncol=2, labels = c("A", "", "")) 
