...
table(df_analysis_severe_stunted$study, df_analysis_severe_stunted$severe_stunted)
...
df_analysis_severe_stunted <- subset(df_analysis_severe_stunted, !(study %in% c("DYADG", "DYADM", "HAITI", "GHANA")))
...
SS.Comb_severe_stunted <- df_analysis_severe_stunted %>%
  ...
  map_dfr(function(df_analysis_severe_stunted) {
    ...
    if (study_name %in% c("DOSE")) {
      model <- gam(severe_stunted ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_severe_stunted,
...
new.data_severe_stunted <- df_analysis_severe_stunted %>%
...
predictions.SS.Comb_severe_stunted <- new.data_severe_stunted %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_severe_stunted, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_severe_stunted)
...
plot_study_severe_stunted <- predictions.SS.Comb_severe_stunted%>%
...
print(plot_study_severe_stunted)
table(df_analysis_severe_stunted$study, df_analysis_severe_stunted$severe_stunted)
...
point.wise.DF.SS.Comb_severe_stunted =  pointwise.ma(predictions.SS.Comb_severe_stunted,
...
point.wise.DF.SS.Comb_severe_stunted$RE.meta  =        expit(point.wise.DF.SS.Comb_severe_stunted$RE.meta)
point.wise.DF.SS.Comb_severe_stunted$RE.meta.upper  =  expit(point.wise.DF.SS.Comb_severe_stunted$RE.meta.upper)
point.wise.DF.SS.Comb_severe_stunted$RE.meta.lower  =  expit(point.wise.DF.SS.Comb_severe_stunted$RE.meta.lower)
...
point.wise.DF.SS.Comb.plot_severe_stunted = point.wise.DF.SS.Comb_severe_stunted%>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_severe_stunted$iwi), max(point.wise.DF.SS.Comb_severe_stunted$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_severe_stunted)
...
predictions.SS.Comb_severe_stunted=predictions.SS.Comb_severe_stunted%>%
...
absolute_diff_SS.Comb_severe_stunted = risk.diff.creator(dataframe = predictions.SS.Comb_severe_stunted,
...
absolute_diff_SS.Comb_severe_stunted=  absolute_diff_SS.Comb_severe_stunted%>%
...
point.wise.absolute_diff_SS.Comb_severe_stunted  =  pointwise.ma(data = absolute_diff_SS.Comb_severe_stunted,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_severe_stunted =  point.wise.absolute_diff_SS.Comb_severe_stunted%>%
...
point.wise.DF.SS.Comb.diff.plot_severe_stunted=point.wise.absolute_diff_SS.Comb_severe_stunted%>%
...
print(point.wise.DF.SS.Comb.diff.plot_severe_stunted)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_severe_stunted=point.wise.absolute_diff_SS.Comb_severe_stunted%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_severe_stunted)

point.wise.pvalQ.plot_severe_stunted=point.wise.absolute_diff_SS.Comb_severe_stunted%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_severe_stunted)


plot_composite_hetero_severe_stunted <- plot_grid(point.wise.Q.plot_severe_stunted, point.wise.pvalQ.plot_severe_stunted,
                               nrow=2, align = "h") 
plot_composite_hetero_severe_stunted
...
plot_comp_effect_severe_stunted <- plot_grid(point.wise.DF.SS.Comb.plot_severe_stunted, point.wise.DF.SS.Comb.diff.plot_severe_stunted, ncol = 1, labels = c("B", "C"))


plot_composite_severe_stunted <- plot_grid(plot_study_severe_stunted, plot_comp_effect_severe_stunted,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_severe_stunted
