---
title: "Absolute treatment effect by Pointwise meta-analysis - stunted"
...
df_analysis_stunted <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
SS.Comb_stunted <- df_analysis_stunted %>%
  split(.$study) %>%
...
new.data_stunted <- df_analysis_stunted %>%
...
predictions.SS.Comb_stunted <- new.data_stunted %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_stunted, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_stunted)
...
plot_study_stunted <- predictions.SS.Comb_stunted%>%
...
point.wise.DF.SS.Comb_stunted =  pointwise.ma(predictions.SS.Comb_stunted,
...
point.wise.DF.SS.Comb_stunted$RE.meta  =        expit(point.wise.DF.SS.Comb_stunted$RE.meta)
point.wise.DF.SS.Comb_stunted$RE.meta.upper  =  expit(point.wise.DF.SS.Comb_stunted$RE.meta.upper)
point.wise.DF.SS.Comb_stunted$RE.meta.lower  =  expit(point.wise.DF.SS.Comb_stunted$RE.meta.lower)
...
point.wise.DF.SS.Comb.plot_stunted = point.wise.DF.SS.Comb_stunted%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled probability of stunting')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_stunted$iwi), max(point.wise.DF.SS.Comb_stunted$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_stunted)
...
predictions.SS.Comb_stunted=predictions.SS.Comb_stunted%>%
...
absolute_diff_SS.Comb_stunted = risk.diff.creator(dataframe = predictions.SS.Comb_stunted,
...
absolute_diff_SS.Comb_stunted=  absolute_diff_SS.Comb_stunted%>%
...
point.wise.absolute_diff_SS.Comb_stunted  =  pointwise.ma(data = absolute_diff_SS.Comb_stunted ,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_stunted =  point.wise.absolute_diff_SS.Comb_stunted%>%
...
point.wise.DF.SS.Comb.diff.plot_stunted=point.wise.absolute_diff_SS.Comb_stunted%>%
...
print(point.wise.DF.SS.Comb.diff.plot_stunted)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_stunted=point.wise.absolute_diff_SS.Comb_stunted%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_stunted)

point.wise.pvalQ.plot_stunted=point.wise.absolute_diff_SS.Comb_stunted%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_stunted)


plot_composite_hetero_stunted <- plot_grid(point.wise.Q.plot_stunted, point.wise.pvalQ.plot_stunted,
                               nrow=2, align = "h") 
plot_composite_hetero_stunted
...
plot_comp_effect_stunted <- plot_grid(point.wise.DF.SS.Comb.plot_stunted, point.wise.DF.SS.Comb.diff.plot_stunted, ncol = 1, labels = c("B", "C"))


plot_composite_stunted <- plot_grid(plot_study_stunted, plot_comp_effect_stunted,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_stunted
