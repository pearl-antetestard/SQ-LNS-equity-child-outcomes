---
title: "Absolute treatment effect by Pointwise meta-analysis - socioemotional score"
...
df_analysis_socioemo <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
SS.Comb_socioemo <- df_analysis_socioemo %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_socioemo) {
    study_name <- unique(df_analysis_socioemo$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(d5_socioemo_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_socioemo,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(d5_socioemo_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_socioemo <- df_analysis_socioemo %>%
...
predictions.SS.Comb_socioemo <- new.data_socioemo %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_socioemo, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_socioemo)
...
plot_study_socioemo <- predictions.SS.Comb_socioemo%>%
...
point.wise.DF.SS.Comb_socioemo =  pointwise.ma(predictions.SS.Comb_socioemo,
...
point.wise.DF.SS.Comb_socioemo$RE.meta  =        point.wise.DF.SS.Comb_socioemo$RE.meta
point.wise.DF.SS.Comb_socioemo$RE.meta.upper  =  point.wise.DF.SS.Comb_socioemo$RE.meta.upper
point.wise.DF.SS.Comb_socioemo$RE.meta.lower  =  point.wise.DF.SS.Comb_socioemo$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_socioemo = point.wise.DF.SS.Comb_socioemo%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled socioemotional score')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_socioemo$iwi),  max(point.wise.DF.SS.Comb_socioemo$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_socioemo)
...
predictions.SS.Comb_socioemo=predictions.SS.Comb_socioemo%>%
...
absolute_diff_SS.Comb_socioemo = risk.diff.creator(dataframe = predictions.SS.Comb_socioemo,
...
absolute_diff_SS.Comb_socioemo=  absolute_diff_SS.Comb_socioemo%>%
...
point.wise.absolute_diff_SS.Comb_socioemo  =  pointwise.ma(data = absolute_diff_SS.Comb_socioemo,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_socioemo =  point.wise.absolute_diff_SS.Comb_socioemo%>%
...
point.wise.DF.SS.Comb.diff.plot_socioemo=point.wise.absolute_diff_SS.Comb_socioemo%>%
...
print(point.wise.DF.SS.Comb.diff.plot_socioemo)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_socioemo=point.wise.absolute_diff_SS.Comb_socioemo%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_socioemo)

point.wise.pvalQ.plot_socioemo=point.wise.absolute_diff_SS.Comb_socioemo%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_socioemo)


plot_composite_hetero_socioemo <- plot_grid(point.wise.Q.plot_socioemo, point.wise.pvalQ.plot_socioemo,
                               nrow=2, align = "h") 
plot_composite_hetero_socioemo
...
plot_comp_effect_socioemo <- plot_grid(point.wise.DF.SS.Comb.plot_socioemo, point.wise.DF.SS.Comb.diff.plot_socioemo, ncol = 1, labels = c("B", "C"))


plot_composite_socioemo <- plot_grid(plot_study_socioemo, plot_comp_effect_socioemo,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_socioemo
