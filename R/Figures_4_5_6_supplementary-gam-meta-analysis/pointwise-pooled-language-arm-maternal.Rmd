---
title: "Absolute treatment effect by Pointwise meta-analysis - language score"
...
df_analysis_language <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
...
SS.Comb_language <- df_analysis_language %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_language) {
    study_name <- unique(df_analysis_language$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(d1_lang_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_language,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(d1_lang_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_language <- df_analysis_language %>%
...
predictions.SS.Comb_language <- new.data_language %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_language, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_language)
...
plot_study_language <- predictions.SS.Comb_language %>%
...
point.wise.DF.SS.Comb_language =  pointwise.ma(predictions.SS.Comb_language,
...
point.wise.DF.SS.Comb_language$RE.meta  =        point.wise.DF.SS.Comb_language$RE.meta
point.wise.DF.SS.Comb_language$RE.meta.upper  =  point.wise.DF.SS.Comb_language$RE.meta.upper
point.wise.DF.SS.Comb_language$RE.meta.lower  =  point.wise.DF.SS.Comb_language$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_language = point.wise.DF.SS.Comb_language%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled language score')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_language$iwi),  max(point.wise.DF.SS.Comb_language$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_language)
...
predictions.SS.Comb_language=predictions.SS.Comb_language%>%
...
absolute_diff_SS.Comb_language = risk.diff.creator(dataframe = predictions.SS.Comb_language,
...
absolute_diff_SS.Comb_language=  absolute_diff_SS.Comb_language%>%
...
point.wise.absolute_diff_SS.Comb_language  =  pointwise.ma(data = absolute_diff_SS.Comb_language,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_language =  point.wise.absolute_diff_SS.Comb_language%>%
...
point.wise.DF.SS.Comb.diff.plot_language=point.wise.absolute_diff_SS.Comb_language%>%
...
print(point.wise.DF.SS.Comb.diff.plot_language)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_language=point.wise.absolute_diff_SS.Comb_language%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_language)

point.wise.pvalQ.plot_language=point.wise.absolute_diff_SS.Comb_language%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_language)


plot_composite_hetero_language <- plot_grid(point.wise.Q.plot_language, point.wise.pvalQ.plot_language,
                               nrow=2, align = "h") 
plot_composite_hetero_language
...
plot_comp_effect_language <- plot_grid(point.wise.DF.SS.Comb.plot_language, point.wise.DF.SS.Comb.diff.plot_language, ncol = 1, labels = c("B", "C"))


plot_composite_language <- plot_grid(plot_study_language, plot_comp_effect_language,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_language
