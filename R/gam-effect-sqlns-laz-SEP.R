### Effect of SQ-LNS on LAZ by wealth between the intervention and control groups # nolint
...
df_analysis <- readRDS(here::here("data", "1-final",
               ...
               select(study, pid, cluster, arm_primary, laz_el, wealth_rank) %>% # nolint
...
# Conduct the gam() analysis for each study with a random effect for only those studies  # nolint
...
mod_gam_dose = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary), # nolint
...
gam_diff_dose <- get_smooths_difference(mod_gam_dose, wealth_rank,
                                        difference = list(arm_primary = c("1","0"))) # nolint
print(gam_diff_dose)

# Plot the results
gam_plot_dose <- plot_smooths(model = mod_gam_dose, series = wealth_rank,
                              comparison = arm_primary) +
                 scale_colour_manual(values = c('#E69F00', 'turquoise'), # nolint
                                     labels = c("Control", "SQ-LNS")) +  # nolint
                 scale_fill_manual(values = c('#E69F00', 'turquoise'), # nolint
                                   labels = c("Control", "SQ-LNS")) +  # nolint # nolint
                 scale_linetype_manual(values=c("solid","dashed"), # nolint
                                       labels = c("Control", "SQ-LNS")) +
                 labs(colour = "Arms", fill = "Arms", linetype = "Arms") + # nolint
                 labs(x = "Wealth rank based on the wealth index score",
                      y = "Mean LAZ (DOSE)"#, tag = "a \n" # nolint
                      ) + # nolint
                 coord_cartesian(ylim=c(-2.5,-1.0)) + # nolint
                 theme_minimal() +
                 theme(legend.position = c(0.75, 0.75),
                       legend.title = element_blank(),
                       legend.key.size = unit(0.3, 'cm'),text = element_text(size=13)) # nolint

plot_diff_dose <- ggplot(gam_diff_dose, aes(wealth_rank, difference)) +
                  geom_hline(aes(yintercept = 0), linetype = "dotted") +  # nolint
                  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha = 0.3) + # nolint
                  geom_line(aes(), linewidth = 0.6) +
                  labs(x = "Wealth rank based on the wealth index score",
                       y = "LAZ mean difference (control as ref)"#, tag = "b \n" # nolint
                       ) + # nolint
                  theme_minimal() + # nolint
                  theme(legend.key.size = unit(0.4, 'cm'),text = element_text(size=13)) # nolint

plot_composite_dose <- ggarrange(gam_plot_dose, plot_diff_dose,  # nolint
...
mod_gam_ghana = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary), # nolint
...
gam_plot_ghana <- plot_smooths(model = mod_gam_ghana, series = wealth_rank,
                  ...
                  coord_cartesian(ylim=c(-1.5,1.0)) + # nolint
                  theme_minimal() +
                  theme(legend.position = c(0.75, 0.75),
                        legend.title = element_blank(),
                        legend.key.size = unit(0.3, 'cm'),text = element_text(size=13)) # nolint
...
mod_gam_haiti = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary), # nolint
...
gam_plot_haiti <- plot_smooths(model = mod_gam_haiti, series = wealth_rank,
                  ...
                  coord_cartesian(ylim=c(-1.5,0.0)) + # nolint
                  theme_minimal() +
                  theme(legend.position = c(0.75, 0.75),
                        legend.title = element_blank(),
                        legend.key.size = unit(0.3, 'cm'),text = element_text(size=13)) # nolint
...
ggsave(here::here("output", "laz_el_wealth_treatment_HAITI.png"),
                  plot = plot_composite_haiti, # nolint
                  width = 5, height = 8)
...
df_analysis_washb <- df_analysis %>%
  ...
  mutate(cluster = factor(cluster),
         dummy = rep(1, n())) # Create a dummy variable for the random effect # nolint

mod_gam_washb = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
...
gam_plot_washb <- plot_smooths(model = mod_gam_washb, series = wealth_rank,
                 ...
                 coord_cartesian(ylim = c(-2.5,-0.5)) + # nolint
                 theme_minimal() +
                 theme(legend.position = c(0.75, 0.75),
                       legend.title = element_blank(),
                       legend.key.size = unit(0.3, 'cm'),text = element_text(size=13))  # nolint
...
mod_gam_jivita = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                     s(cluster, bs = "re", by = dummy), # re is on # nolint
...
mod_gam_mahay = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                    s(cluster, bs = "re", by = dummy), # re is on # nolint
...
gam_plot_mahay <- plot_smooths(model = mod_gam_mahay, series = wealth_rank,
                 ...
                 coord_cartesian(ylim=c(-2.5,-1.8)) + # nolint
                 theme_minimal() +
                 theme(legend.position = c(0.75, 0.75),
                       legend.title = element_blank(),
                       legend.key.size = unit(0.3, 'cm'),text = element_text(size=13))   # nolint
...
mod_gam_promisbf = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                    s(cluster, bs = "re", by = dummy), # re is on # nolint
                    data = df_analysis_promisbf, # nolint
                    family = "gaussian", method = "REML")
...
# Plot the results
gam_plot_promisbf <- plot_smooths(model = mod_gam_promisbf, series = wealth_rank,  # nolint
                                  comparison = arm_primary) +
                     scale_colour_manual(values = c('#E69F00', 'turquoise'), # nolint
                                         labels = c("Control", "SQ-LNS")) +
                     scale_fill_manual(values = c('#E69F00', 'turquoise'), # nolint
                 ...
                 coord_cartesian(ylim = c(-1.7, -1.0)) + # nolint
                 theme_minimal() +
...
plot_composite_promisbf <- ggarrange(gam_plot_promisbf, plot_diff_promisbf,
                                     ncol = 1, align = "h", labels = c("A", "B")) # nolint
...
mod_gam_promisbf_cs = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                      s(cluster, bs = "re", by = dummy), # re is on # nolint
                      data = df_analysis_promisbf_cs, # nolint
                      family = "gaussian", method = "REML")
summary(mod_gam_promisbf_cs)

# Get the smooth difference
gam_diff_promisbf_cs <- get_smooths_difference(mod_gam_promisbf_cs, wealth_rank,  # nolint
                                               difference = list(arm_primary = c("1","0"))) # nolint
print(gam_diff_promisbf_cs)

# Plot the results
gam_plot_promisbf_cs <- plot_smooths(model = mod_gam_promisbf_cs, series = wealth_rank,  # nolint
                                     comparison = arm_primary) +
                        scale_colour_manual(values = c('#E69F00', 'turquoise'), # nolint
                                            labels = c("Control", "SQ-LNS")) +
                        scale_fill_manual(values = c('#E69F00', 'turquoise'), # nolint
                        ...
                        coord_cartesian(ylim=c(-2.0,-0.5)) + # nolint
                        theme_minimal() +
                        theme(legend.position = c(0.75, 0.75),
                        legend.title = element_blank(), # nolint
                        legend.key.size = unit(0.3, 'cm'),text = element_text(size=13)) # nolint

plot_diff_promisbf_cs <- ggplot(gam_diff_promisbf_cs, aes(wealth_rank, difference)) + # nolint
                         geom_hline(aes(yintercept = 0), linetype = "dotted") +  # nolint
                         geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha = 0.3) + # nolint
                         geom_line(aes(), linewidth = 0.6) +
                         labs(x = "Wealth rank based on the wealth index score",
...
plot_composite_promisbf_cs <- ggarrange(gam_plot_promisbf_cs, plot_diff_promisbf_cs,  # nolint
                                        ncol = 1, align = "h", labels = c("A", "B")) # nolint
...
mod_gam_promism = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                  s(cluster, bs = "re", by = dummy), # re is on # nolint
                    data = df_analysis_promism, # nolint
                    family = "gaussian", method = "REML")
...
gam_plot_promism <- plot_smooths(model = mod_gam_promism, series = wealth_rank,
                    ...
                    coord_cartesian(ylim = c(-2.0, -1.0)) + # nolint
                    theme_minimal() +
                    theme(legend.position = c(0.75, 0.75),
                          legend.title = element_blank(),
                          legend.key.size = unit(0.3, 'cm'),text = element_text(size=13)) # nolint
...
mod_gam_promism_cs = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                     s(cluster, bs = "re", by = dummy), # re is on # nolint
                     data = df_analysis_promism_cs, # nolint
                     family = "gaussian", method = "REML")
...
# Plot the results
gam_plot_promism_cs <- plot_smooths(model = mod_gam_promism_cs, series = wealth_rank,  # nolint
                                    comparison = arm_primary) +
                       scale_colour_manual(values = c('#E69F00', 'turquoise'), # nolint
                                           labels = c("Control", "SQ-LNS")) +
                       scale_fill_manual(values = c('#E69F00', 'turquoise'), # nolint
...
plot_diff_promism_cs <- ggplot(gam_diff_promism_cs, aes(wealth_rank, difference)) + # nolint
                        geom_hline(aes(yintercept = 0), linetype = "dotted") +  # nolint
                        geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha = 0.3) + # nolint
                        geom_line(aes(), linewidth = 0.6) +
                        labs(x = "Wealth rank based on the wealth index score", # nolint
                             y = "LAZ mean difference (control as ref)"#, tag = "b \n" # nolint
                             ) + # nolint
                        theme_minimal() + # nolint
                        theme(legend.key.size = unit(0.4, 'cm'),text = element_text(size=13)) # nolint

plot_composite_promism_cs <- ggarrange(gam_plot_promism_cs, plot_diff_promism_cs,  # nolint
                                       ncol=1, align = "h", labels = c("A", "B")) # nolint
...
mod_gam_rdns = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
               s(cluster, bs = "re", by = dummy), # re is on # nolint
               data = df_analysis_rdns, # nolint
               family = "gaussian", method = "REML")
...
mod_gam_shine_hiv_neg = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                        s(cluster, bs = "re", by = dummy), # re is on # nolint
                        data = df_analysis_shine_hiv_neg, # nolint
                        family = "gaussian", method = "REML")
summary(mod_gam_shine_hiv_neg)

# Get the smooth difference
gam_diff_shine_hiv_neg <- get_smooths_difference(mod_gam_shine_hiv_neg, wealth_rank,  # nolint
                                                 difference = list(arm_primary = c("1","0"))) # nolint
print(gam_diff_shine_hiv_neg)

# Plot the results
gam_plot_shine_hiv_neg <- plot_smooths(model = mod_gam_shine_hiv_neg, series = wealth_rank,  # nolint
                                       comparison = arm_primary) +
                          scale_colour_manual(values = c('#E69F00', 'turquoise'), # nolint
                                              labels = c("Control", "SQ-LNS")) +  # nolint
                          scale_fill_manual(values = c('#E69F00', 'turquoise'), # nolint
...
plot_diff_shine_hiv_neg <- ggplot(gam_diff_shine_hiv_neg, aes(wealth_rank, difference)) + # nolint
                           geom_hline(aes(yintercept = 0), linetype = "dotted") +  # nolint
                           geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha = 0.3) + # nolint
                           geom_line(aes(), linewidth = 0.6) +
                           labs(x = "Wealth rank based on the wealth index score", # nolint
...
plot_composite_shine_hiv_neg <- ggarrange(gam_plot_shine_hiv_neg, plot_diff_shine_hiv_neg,  # nolint
                                ncol = 1, align = "h", labels = c("A", "B")) # nolint
...
mod_gam_shine_hiv_pos = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                        s(cluster, bs = "re", by = dummy), # re is on # nolint
                        data = df_analysis_shine_hiv_pos, # nolint
                        family = "gaussian", method = "REML")
summary(mod_gam_shine_hiv_pos)

# Get the smooth difference
gam_diff_shine_hiv_pos <- get_smooths_difference(mod_gam_shine_hiv_pos, wealth_rank,  # nolint
                          difference = list(arm_primary = c("1","0"))) # nolint
print(gam_diff_shine_hiv_pos)

# Plot the results
gam_plot_shine_hiv_pos <- plot_smooths(model = mod_gam_shine_hiv_pos, series = wealth_rank,  # nolint
                                       comparison = arm_primary) +
                          scale_colour_manual(values = c('#E69F00', 'turquoise'), # nolint
                                              labels = c("Control", "SQ-LNS")) +  # nolint
                          scale_fill_manual(values = c('#E69F00', 'turquoise'), # nolint
                 ...
                 coord_cartesian(ylim = c(-2.75, -1.25)) + # nolint
                 theme_minimal() +
                 theme(legend.position = c(0.75, 0.75),
                       legend.title = element_blank(),
                       legend.key.size = unit(0.3, 'cm'), text = element_text(size = 13)) # nolint

plot_diff_shine_hiv_pos <- ggplot(gam_diff_shine_hiv_pos, aes(wealth_rank, difference)) + # nolint
                           geom_hline(aes(yintercept = 0), linetype = "dotted") +  # nolint
                           geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha = 0.3) + # nolint
                           geom_line(aes(), linewidth = 0.6) +
                           labs(x = "Wealth rank based on the wealth index score", # nolint
...
plot_composite_shine_hiv_pos <- ggarrange(gam_plot_shine_hiv_pos, plot_diff_shine_hiv_pos,  # nolint
                                          ncol=1, align = "h", labels = c("A", "B")) # nolint
...
mod_gam_washk = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
                s(cluster, bs = "re", by = dummy), # re is on # nolint
...
mod_gam_zinc = gam(laz_el ~ arm_primary + s(wealth_rank, bs = "cr", by = arm_primary) + # nolint
               s(cluster, bs = "re", by = dummy), # re is on # nolint
               data = df_analysis_zinc, # nolint
               family = "gaussian", method = "REML")
