---
title: "Absolute treatment effect by Pointwise meta-analysis - hemoglobin concentration, female"
...
df_analysis_hgb_f <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 1) %>%
  dplyr::mutate(study = sub("s[0-9]*_", "", study.x)) %>%
  select(study, pid, region, cluster, arms_maternal, iwi, b1_hb_el) %>% 
  dplyr::filter(!is.na(iwi)) %>%
  dplyr::filter(!is.na(arms_maternal)) %>%
  dplyr::filter(!is.na(b1_hb_el)) %>%
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(b1_hb_el ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb_hgb_f <- df_analysis_hgb_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_hgb_f) {
    study_name <- unique(df_analysis_hgb_f$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(b1_hb_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_hgb_f,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(b1_hb_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_hgb_f <- df_analysis_hgb_f %>%
...
predictions.SS.Comb_hgb_f <- new.data_hgb_f %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_hgb_f, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_hgb_f)
...
plot_study_hgb_f <- predictions.SS.Comb_hgb_f %>%
  ...
  ylab("Hemoglobin concentration (female)") +
  xlab("IWI") +
...
point.wise.DF.SS.Comb_hgb_f =  pointwise.ma(predictions.SS.Comb_hgb_f,
...
point.wise.DF.SS.Comb_hgb_f$RE.meta  =        point.wise.DF.SS.Comb_hgb_f$RE.meta
point.wise.DF.SS.Comb_hgb_f$RE.meta.upper  =  point.wise.DF.SS.Comb_hgb_f$RE.meta.upper
point.wise.DF.SS.Comb_hgb_f$RE.meta.lower  =  point.wise.DF.SS.Comb_hgb_f$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_hgb_f = point.wise.DF.SS.Comb_hgb_f %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled hemoglobin concentration (female)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_hgb_f$iwi),  max(point.wise.DF.SS.Comb_hgb_f$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_hgb_f)
...
predictions.SS.Comb_hgb_f=predictions.SS.Comb_hgb_f%>%
...
absolute_diff_SS.Comb_hgb_f = risk.diff.creator(dataframe = predictions.SS.Comb_hgb_f,
...
absolute_diff_SS.Comb_hgb_f =  absolute_diff_SS.Comb_hgb_f%>%
...
point.wise.absolute_diff_SS.Comb_hgb_f  =  pointwise.ma(data = absolute_diff_SS.Comb_hgb_f,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_hgb_f =  point.wise.absolute_diff_SS.Comb_hgb_f%>%
...
point.wise.DF.SS.Comb.diff.plot_hgb_f=point.wise.absolute_diff_SS.Comb_hgb_f%>%
  ...
  theme(axis.title.y = element_text(size = 18),
        ...
        axis.text.y = element_text(face="bold",  size=12),
        axis.text.x = element_text(face="bold",  size=12))  +
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_hgb_f$iwi),  max(point.wise.absolute_diff_SS.Comb_hgb_f$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_hgb_f)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_hgb_f=point.wise.absolute_diff_SS.Comb_hgb_f%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_hgb_f)

point.wise.pvalQ.plot_hgb_f=point.wise.absolute_diff_SS.Comb_hgb_f%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_hgb_f)


plot_composite_hetero_hgb_f <- plot_grid(point.wise.Q.plot_hgb_f, point.wise.pvalQ.plot_hgb_f,
                               nrow=2, align = "h") 
plot_composite_hetero_hgb_f
...
plot_comp_effect_hgb_f <- plot_grid(point.wise.DF.SS.Comb.plot_hgb_f, point.wise.DF.SS.Comb.diff.plot_hgb_f, ncol = 1, labels = c("B", "C"))


plot_composite_hgb_f <- plot_grid(plot_study_hgb_f, plot_comp_effect_hgb_f,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_hgb_f
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_hemoglobin_arm_maternal.png"),
    #   plot = plot_composite_hgb, width = 14, height = 16, dpi = 300)
```
