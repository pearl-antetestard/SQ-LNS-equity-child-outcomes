---
title: "Absolute treatment effect by Pointwise meta-analysis - wasted, female"
...
df_analysis_wasted_f <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 1) %>%
...
table(df_analysis_wasted_f$study, df_analysis_wasted_f$wasted_el)


```


Note: DYADG and DYADM have empty rows for arm_primary -- why? These are the studies that had maternal interventions. Removing them from the analysis.Also removed HAITI and PROMISM because of very small sample sizes for wasting. 

```{r 1, echo = TRUE, warning = FALSE}

df_analysis_wasted_f <- subset(df_analysis_wasted_f, !(study %in% c("HAITI", "PROMISM")))
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(wasted_el ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb_wasted_f <- df_analysis_wasted_f %>%
  dplyr::mutate(study = factor(study)) %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wasted_f) {
    study_name <- unique(df_analysis_wasted_f$study)
    if (study_name %in% c("DOSE", "GHANA","DYADG","DYADM")) {
      model <- gam(wasted_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_wasted_f,
...
new.data_wasted_f <- df_analysis_wasted_f %>%
...
predictions.SS.Comb_wasted_f <- new.data_wasted_f %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_wasted_f, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_wasted_f)
...
plot_study_wasted_f <- predictions.SS.Comb_wasted_f %>%
  ...
  ylab("Probability of wasting (female)") +
  xlab("IWI") +
...
point.wise.DF.SS.Comb_wasted_f =  pointwise.ma(predictions.SS.Comb_wasted_f,
...
point.wise.DF.SS.Comb_wasted_f$RE.meta  =        expit(point.wise.DF.SS.Comb_wasted_f$RE.meta)
point.wise.DF.SS.Comb_wasted_f$RE.meta.upper  =  expit(point.wise.DF.SS.Comb_wasted_f$RE.meta.upper)
point.wise.DF.SS.Comb_wasted_f$RE.meta.lower  =  expit(point.wise.DF.SS.Comb_wasted_f$RE.meta.lower)
...
point.wise.DF.SS.Comb.plot_wasted_f = point.wise.DF.SS.Comb_wasted_f %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled probability of wasting (female)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_wasted_f$iwi), max(point.wise.DF.SS.Comb_wasted_f$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_wasted_f)
...
predictions.SS.Comb_wasted_f =predictions.SS.Comb_wasted_f %>%
...
absolute_diff_SS.Comb_wasted_f = risk.diff.creator(dataframe = predictions.SS.Comb_wasted_f,
...
absolute_diff_SS.Comb_wasted_f =  absolute_diff_SS.Comb_wasted_f %>%
...
point.wise.absolute_diff_SS.Comb_wasted_f  =  pointwise.ma(data = absolute_diff_SS.Comb_wasted_f,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_wasted_f =  point.wise.absolute_diff_SS.Comb_wasted_f %>%
...
point.wise.DF.SS.Comb.diff.plot_wasted_f =point.wise.absolute_diff_SS.Comb_wasted_f %>%
  ...
  theme(axis.title.y = element_text(size = 18),
        ...
        strip.text = element_text(face="bold", size=14, hjust = 0.5),
        axis.text.y = element_text(face="bold",  size=12),
        axis.text.x = element_text(face="bold",  size=12)) + ylim(c(-0.05,0.1)) +
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_wasted_f$iwi), max(point.wise.absolute_diff_SS.Comb_wasted_f$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_wasted_f)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_wasted_f =point.wise.absolute_diff_SS.Comb_wasted_f %>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_wasted_f)

point.wise.pvalQ.plot_wasted_f = point.wise.absolute_diff_SS.Comb_wasted_f %>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_wasted_f)


plot_composite_hetero_wasted_f <- plot_grid(point.wise.Q.plot_wasted_f, point.wise.pvalQ.plot_wasted_f,
                               nrow=2, align = "h") 
plot_composite_hetero_wasted_f
...
plot_comp_effect_wasted_f <- plot_grid(point.wise.DF.SS.Comb.plot_wasted_f, point.wise.DF.SS.Comb.diff.plot_wasted_f, ncol = 1, labels = c("B", "C"))


plot_composite_wasted_f <- plot_grid(plot_study_wasted_f, plot_comp_effect_wasted_f,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_wasted_f
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_wasted_arm_maternal.png"),
#       plot = plot_composite_wasted, width = 14, height = 16, dpi = 300)
```
