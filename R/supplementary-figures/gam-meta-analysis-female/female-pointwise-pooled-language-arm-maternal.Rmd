---
title: "Absolute treatment effect by Pointwise meta-analysis - language score, female"
...
df_analysis_language_f <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>%
  subset(female == 1) %>%
  dplyr::mutate(study = sub("s[0-9]*_", "", study.x)) %>%
  select(study, pid, region, cluster, arms_maternal, iwi, d1_lang_el) %>% 
  dplyr::filter(!is.na(iwi)) %>%
  dplyr::filter(!is.na(arms_maternal)) %>%
  dplyr::filter(!is.na(d1_lang_el)) %>%
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(d1_lang_el ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb_language_f <- df_analysis_language_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_language_f) {
    study_name <- unique(df_analysis_language_f$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(d1_lang_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_language_f,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(d1_lang_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_language_f <- df_analysis_language_f %>%
...
predictions.SS.Comb_language_f <- new.data_language_f %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_language_f, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_language_f)
...
plot_study_language_f <- predictions.SS.Comb_language_f %>%
...
point.wise.DF.SS.Comb_language_f =  pointwise.ma(predictions.SS.Comb_language_f,
...
point.wise.DF.SS.Comb_language_f$RE.meta  =        point.wise.DF.SS.Comb_language_f$RE.meta
point.wise.DF.SS.Comb_language_f$RE.meta.upper  =  point.wise.DF.SS.Comb_language_f$RE.meta.upper
point.wise.DF.SS.Comb_language_f$RE.meta.lower  =  point.wise.DF.SS.Comb_language_f$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_language_f = point.wise.DF.SS.Comb_language_f %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled language score (female)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_language_f$iwi),  max(point.wise.DF.SS.Comb_language_f$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_language_f)
...
predictions.SS.Comb_language_f = predictions.SS.Comb_language_f %>%
...
absolute_diff_SS.Comb_language_f = risk.diff.creator(dataframe = predictions.SS.Comb_language_f,
...
absolute_diff_SS.Comb_language_f =  absolute_diff_SS.Comb_language_f %>%
...
point.wise.absolute_diff_SS.Comb_language_f  =  pointwise.ma(data = absolute_diff_SS.Comb_language_f,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_language_f =  point.wise.absolute_diff_SS.Comb_language_f %>%
...
point.wise.DF.SS.Comb.diff.plot_language_f = point.wise.absolute_diff_SS.Comb_language_f %>%
  ...
  theme(axis.title.y = element_text(size = 18),
        ...
        strip.text = element_text(face="bold", size=14, hjust = 0.5),
        axis.text.y = element_text(face="bold",  size=12),
        axis.text.x = element_text(face="bold",  size=12)) + ylim(c(-0.2,0.4)) +
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_language_f$iwi),  max(point.wise.absolute_diff_SS.Comb_language_f$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_language_f)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_language_f = point.wise.absolute_diff_SS.Comb_language_f %>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_language_f)

point.wise.pvalQ.plot_language_f = point.wise.absolute_diff_SS.Comb_language_f %>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_language_f)


plot_composite_hetero_language_f <- plot_grid(point.wise.Q.plot_language_f, point.wise.pvalQ.plot_language_f,
                               nrow=2, align = "h") 
plot_composite_hetero_language_f
...
plot_comp_effect_language_f <- plot_grid(point.wise.DF.SS.Comb.plot_language_f, point.wise.DF.SS.Comb.diff.plot_language_f, ncol = 1, labels = c("B", "C"))


plot_composite_language_f <- plot_grid(plot_study_language_f, plot_comp_effect_language_f,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_language_f
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_d1_lang_el_arm_maternal.png"),
    #   plot = plot_composite_language, width = 14, height = 16, dpi = 300)
```
