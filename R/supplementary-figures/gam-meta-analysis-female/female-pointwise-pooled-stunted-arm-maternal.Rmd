---
title: "Absolute treatment effect by Pointwise meta-analysis - stunted, female"
...
df_analysis_stunted_f <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 1) %>%
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(stunted_el ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb_stunted_f <- df_analysis_stunted_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_stunted_f) {
    study_name <- unique(df_analysis_stunted_f$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(stunted_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_stunted_f,
...
new.data_stunted_f <- df_analysis_stunted_f %>%
...
predictions.SS.Comb_stunted_f <- new.data_stunted_f %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_stunted_f, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_stunted_f)
...
plot_study_stunted_f <- predictions.SS.Comb_stunted_f%>%
  ggplot(., aes(iwi,expit(.fitted), color= arms_maternal))+ 
  geom_line(size=1.25)+ 
 facet_wrap(.~study, ncol = 2, nrow = 8) + 
  ...
  ylab("Probability of stunting (female)") +
  xlab("IWI") +
...
point.wise.DF.SS.Comb_stunted_f =  pointwise.ma(predictions.SS.Comb_stunted_f,
...
point.wise.DF.SS.Comb_stunted_f$RE.meta  =        expit(point.wise.DF.SS.Comb_stunted_f$RE.meta)
point.wise.DF.SS.Comb_stunted_f$RE.meta.upper  =  expit(point.wise.DF.SS.Comb_stunted_f$RE.meta.upper)
point.wise.DF.SS.Comb_stunted_f$RE.meta.lower  =  expit(point.wise.DF.SS.Comb_stunted_f$RE.meta.lower)
...
point.wise.DF.SS.Comb.plot_stunted_f = point.wise.DF.SS.Comb_stunted_f %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled probability of stunting (female)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_stunted_f$iwi), max(point.wise.DF.SS.Comb_stunted_f$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_stunted_f)
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_stunted.png"),
...
predictions.SS.Comb_stunted_f =predictions.SS.Comb_stunted_f %>%
...
absolute_diff_SS.Comb_stunted_f = risk.diff.creator(dataframe = predictions.SS.Comb_stunted_f,
...
absolute_diff_SS.Comb_stunted_f =  absolute_diff_SS.Comb_stunted_f %>%
...
point.wise.absolute_diff_SS.Comb_stunted_f  =  pointwise.ma(data = absolute_diff_SS.Comb_stunted_f,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_stunted_f =  point.wise.absolute_diff_SS.Comb_stunted_f %>%
...
point.wise.DF.SS.Comb.diff.plot_stunted_f =point.wise.absolute_diff_SS.Comb_stunted_f %>%
  ...
  theme(axis.title.y = element_text(size = 18),
        ...
        strip.text = element_text(face="bold", size=14, hjust = 0.5),
        axis.text.y = element_text(face="bold",  size=12),
        axis.text.x = element_text(face="bold",  size=12)) + ylim(c(-0.15,0.05)) +
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_stunted_f$iwi), max(point.wise.absolute_diff_SS.Comb_stunted_f$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_stunted_f)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_stunted_f =point.wise.absolute_diff_SS.Comb_stunted_f %>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_stunted_f)

point.wise.pvalQ.plot_stunted_f =point.wise.absolute_diff_SS.Comb_stunted_f %>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_stunted_f)


plot_composite_hetero_stunted_f <- plot_grid(point.wise.Q.plot_stunted_f,
                                             point.wise.pvalQ.plot_stunted_f,
                               nrow=2, align = "h") 
plot_composite_hetero_stunted_f
...
# Save the plot
#ggsave(here::here("output", "qplot_pooled_pointwise_stunted.png"),
...
plot_comp_effect_stunted_f <- plot_grid(point.wise.DF.SS.Comb.plot_stunted_f, point.wise.DF.SS.Comb.diff.plot_stunted_f, ncol = 1, labels = c("B", "C"))


plot_composite_stunted_f <- plot_grid(plot_study_stunted_f, plot_comp_effect_stunted_f,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_stunted_f
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_stunted_arm_maternal.png"),
#       plot = plot_composite_stunted, width = 14, height = 16, dpi = 300)
```
