---
title: "Absolute treatment effect by Pointwise meta-analysis - gross motor score, female"
...
df_analysis_gross_f <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 1) %>%
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(d8_grossmotor_el ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb_gross_f <- df_analysis_gross_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_gross_f) {
    study_name <- unique(df_analysis_gross_f$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(d8_grossmotor_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_gross_f,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(d8_grossmotor_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_gross_f <- df_analysis_gross_f %>%
...
predictions.SS.Comb_gross_f <- new.data_gross_f %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_gross_f, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_gross_f)
...
plot_study_gross_f <- predictions.SS.Comb_gross_f %>%
...
point.wise.DF.SS.Comb_gross_f =  pointwise.ma(predictions.SS.Comb_gross_f,
...
point.wise.DF.SS.Comb_gross_f$RE.meta  =        
  point.wise.DF.SS.Comb_gross_f$RE.meta
point.wise.DF.SS.Comb_gross_f$RE.meta.upper  =  point.wise.DF.SS.Comb_gross_f$RE.meta.upper
point.wise.DF.SS.Comb_gross_f$RE.meta.lower  =  point.wise.DF.SS.Comb_gross_f$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_gross_f = point.wise.DF.SS.Comb_gross_f %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled gross motor score (female)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_gross_f$iwi),  max(point.wise.DF.SS.Comb_gross_f$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_gross_f)
...
predictions.SS.Comb_gross_f=predictions.SS.Comb_gross_f%>%
...
absolute_diff_SS.Comb_gross_f = risk.diff.creator(dataframe = predictions.SS.Comb_gross_f,
...
absolute_diff_SS.Comb_gross_f =  absolute_diff_SS.Comb_gross_f %>%
...
point.wise.absolute_diff_SS.Comb_gross_f  =  pointwise.ma(data = absolute_diff_SS.Comb_gross_f,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_gross_f =  point.wise.absolute_diff_SS.Comb_gross_f %>%
...
point.wise.DF.SS.Comb.diff.plot_gross_f=point.wise.absolute_diff_SS.Comb_gross_f%>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_gross_f$iwi),  max(point.wise.absolute_diff_SS.Comb_gross_f$iwi), by = 10))


print(point.wise.DF.SS.Comb.diff.plot_gross_f)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_gross_f=point.wise.absolute_diff_SS.Comb_gross_f%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_gross_f)

point.wise.pvalQ.plot_gross_f=point.wise.absolute_diff_SS.Comb_gross_f%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_gross_f)


plot_composite_hetero_gross_f <- plot_grid(point.wise.Q.plot_gross_f, point.wise.pvalQ.plot_gross_f,
                               nrow=2, align = "h") 
plot_composite_hetero_gross_f
...
plot_comp_effect_gross_f <- plot_grid(point.wise.DF.SS.Comb.plot_gross_f, point.wise.DF.SS.Comb.diff.plot_gross_f, ncol = 1, labels = c("B", "C"))


plot_composite_gross_f <- plot_grid(plot_study_gross_f, plot_comp_effect_gross_f,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_gross_f
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_d8_grossmotor_el_arm_maternal.png"),
  #     plot = plot_composite_gross, width = 14, height = 16, dpi = 300)
```
