---
title: "Absolute treatment effect by Pointwise meta-analysis - laz, female"
...
df_analysis_laz_f <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 1) %>%
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(laz_el ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb_laz_f <- df_analysis_laz_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_laz_f) {
    study_name <- unique(df_analysis_laz_f$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(laz_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_laz_f,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(laz_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_laz_f <- df_analysis_laz_f %>%
...
predictions.SS.Comb_laz_f <- new.data_laz_f %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_laz_f, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_laz_f)
...
plot_study_laz_f <- predictions.SS.Comb_laz_f %>%
...
point.wise.DF.SS.Comb_laz_f =  pointwise.ma(predictions.SS.Comb_laz_f,
...
point.wise.DF.SS.Comb_laz_f$RE.meta  =        point.wise.DF.SS.Comb_laz_f$RE.meta
point.wise.DF.SS.Comb_laz_f$RE.meta.upper  =  point.wise.DF.SS.Comb_laz_f$RE.meta.upper
point.wise.DF.SS.Comb_laz_f$RE.meta.lower  =  point.wise.DF.SS.Comb_laz_f$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_laz_f = point.wise.DF.SS.Comb_laz_f %>%
...
scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_laz_f$iwi),   max(point.wise.DF.SS.Comb_laz_f$iwi), by = 10))




print(point.wise.DF.SS.Comb.plot_laz_f)
...
predictions.SS.Comb_laz_f =predictions.SS.Comb_laz_f %>%
...
absolute_diff_SS.Comb_laz_f = risk.diff.creator(dataframe = predictions.SS.Comb_laz_f,
...
absolute_diff_SS.Comb_laz_f =  absolute_diff_SS.Comb_laz_f %>%
...
point.wise.absolute_diff_SS.Comb_laz_f  =  pointwise.ma(data = absolute_diff_SS.Comb_laz_f,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_laz_f =  point.wise.absolute_diff_SS.Comb_laz_f %>%
...
point.wise.DF.SS.Comb.diff.plot_laz_f =point.wise.absolute_diff_SS.Comb_laz_f %>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_laz_f$iwi),  
                                  max(point.wise.absolute_diff_SS.Comb_laz_f$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_laz_f)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_laz_f =point.wise.absolute_diff_SS.Comb_laz_f %>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_laz_f)

point.wise.pvalQ.plot_laz_f =point.wise.absolute_diff_SS.Comb_laz_f %>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_laz_f)


plot_composite_hetero_laz_f <- plot_grid(point.wise.Q.plot_laz_f, point.wise.pvalQ.plot_laz_f,
                               nrow=2, align = "h") 
plot_composite_hetero_laz_f
...
plot_comp_effect_laz_f <- plot_grid(point.wise.DF.SS.Comb.plot_laz_f, point.wise.DF.SS.Comb.diff.plot_laz_f, ncol = 1, labels = c("B", "C"))


plot_composite_laz_f <- plot_grid(plot_study_laz_f, plot_comp_effect_laz_f,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_laz_f
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_laz_arm_maternal.png"),
   #    plot = plot_composite_laz, width = 14, height = 16, dpi = 300)
```
