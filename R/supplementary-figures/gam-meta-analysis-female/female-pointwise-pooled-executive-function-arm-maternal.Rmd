---
title: "Absolute treatment effect by Pointwise meta-analysis - executive function score, female"
...
SS.Comb_exec_f <- df_analysis_exec_f %>%
  split(.$study) %>%
...
new.data_exec_f <- df_analysis_exec_f %>%
...
predictions.SS.Comb_exec_f <- new.data_exec_f %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_exec_f, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_exec_f)
...
plot_study_exec_f <- predictions.SS.Comb_exec_f%>%
  ...
  ylab("Executive function score (female)") +
  xlab("IWI") +
...
point.wise.DF.SS.Comb_exec_f =  pointwise.ma(predictions.SS.Comb_exec_f,
...
point.wise.DF.SS.Comb_exec_f$RE.meta  =        
  point.wise.DF.SS.Comb_exec_f$RE.meta
point.wise.DF.SS.Comb_exec_f$RE.meta.upper  =  point.wise.DF.SS.Comb_exec_f$RE.meta.upper
point.wise.DF.SS.Comb_exec_f$RE.meta.lower  =  point.wise.DF.SS.Comb_exec_f$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_exec_f = point.wise.DF.SS.Comb_exec_f%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled executive function score (female)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_exec_f$iwi),  max(point.wise.DF.SS.Comb_exec_f$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_exec_f)
...
predictions.SS.Comb_exec_f=predictions.SS.Comb_exec_f%>%
...
absolute_diff_SS.Comb_exec_f = risk.diff.creator(dataframe = predictions.SS.Comb_exec_f,
...
absolute_diff_SS.Comb_exec_f=  absolute_diff_SS.Comb_exec_f%>%
...
point.wise.absolute_diff_SS.Comb_exec_f  =  pointwise.ma(data = absolute_diff_SS.Comb_exec_f,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_exec_f =  point.wise.absolute_diff_SS.Comb_exec_f%>%
...
point.wise.DF.SS.Comb.diff.plot_exec_f=point.wise.absolute_diff_SS.Comb_exec_f%>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_exec_f$iwi),  max(point.wise.absolute_diff_SS.Comb_exec_f$iwi), by = 10))


print(point.wise.DF.SS.Comb.diff.plot_exec_f)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_exec_f=point.wise.absolute_diff_SS.Comb_exec_f%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_exec_f)

point.wise.pvalQ.plot_exec_f=point.wise.absolute_diff_SS.Comb_exec_f%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_exec_f)


plot_composite_hetero_exec_f <- plot_grid(point.wise.Q.plot_exec_f, point.wise.pvalQ.plot_exec_f,
                               nrow=2, align = "h") 
plot_composite_hetero_exec_f
...
plot_comp_effect_exec_f <- plot_grid(point.wise.DF.SS.Comb.plot_exec_f, point.wise.DF.SS.Comb.diff.plot_exec_f, ncol = 1, labels = c("B", "C"))


plot_composite_exec_f <- plot_grid(plot_study_exec_f, plot_comp_effect_exec_f,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_exec_f
