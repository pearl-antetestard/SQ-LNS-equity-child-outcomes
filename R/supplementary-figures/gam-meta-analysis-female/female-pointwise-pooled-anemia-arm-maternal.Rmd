...
df_analysis_anemia_f <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 1) %>%
...
SS.Comb_anemia_f <- df_analysis_anemia_f %>%
  split(.$study) %>%
...
new.data_anemia_f <- df_analysis_anemia_f %>%
...
predictions.SS.Comb_anemia_f <- new.data_anemia_f %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_anemia_f, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_fit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_anemia_f)
...
plot_study_anemia_f <- predictions.SS.Comb_anemia_f%>%
...
print(plot_study_anemia_f)
table(df_analysis_anemia_f$study, df_analysis_anemia_f$b2_anemia_el)
...
point.wise.DF.SS.Comb_anemia_f =  pointwise.ma(predictions.SS.Comb_anemia_f,
...
point.wise.DF.SS.Comb_anemia_f$RE.meta  =        expit(point.wise.DF.SS.Comb_anemia_f$RE.meta)
point.wise.DF.SS.Comb_anemia_f$RE.meta.upper  =  expit(point.wise.DF.SS.Comb_anemia_f$RE.meta.upper)
point.wise.DF.SS.Comb_anemia_f$RE.meta.lower  =  expit(point.wise.DF.SS.Comb_anemia_f$RE.meta.lower)
...
point.wise.DF.SS.Comb.plot_anemia_f = point.wise.DF.SS.Comb_anemia_f %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled probability of anemia (female)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_anemia_f$iwi),  max(point.wise.DF.SS.Comb_anemia_f$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_anemia_f)
...
predictions.SS.Comb_anemia_f=predictions.SS.Comb_anemia_f%>%
...
absolute_diff_SS.Comb_anemia_f = risk.diff.creator(dataframe = predictions.SS.Comb_anemia_f,
...
absolute_diff_SS.Comb_anemia_f=  absolute_diff_SS.Comb_anemia_f%>%
...
point.wise.absolute_diff_SS.Comb_anemia_f  =  pointwise.ma(data = absolute_diff_SS.Comb_anemia_f,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_anemia_f =  
  point.wise.absolute_diff_SS.Comb_anemia_f %>%
...
point.wise.DF.SS.Comb.diff.plot_anemia_f=point.wise.absolute_diff_SS.Comb_anemia_f%>%
  ...
  theme(axis.title.y = element_text(size = 18),
        ...
        axis.text.y = element_text(face="bold",  size=12),
        axis.text.x = element_text(face="bold",  size=12)) +
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_anemia_f$iwi),  max(point.wise.absolute_diff_SS.Comb_anemia_f$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_anemia_f)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_anemia_f=point.wise.absolute_diff_SS.Comb_anemia_f%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_anemia_f)

point.wise.pvalQ.plot_anemia_f=point.wise.absolute_diff_SS.Comb_anemia_f%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_anemia_f)


plot_composite_hetero_anemia_f <- plot_grid(point.wise.Q.plot_anemia_f, point.wise.pvalQ.plot_anemia_f,
                               nrow=2, align = "h") 
plot_composite_hetero_anemia_f
...
plot_comp_effect_anemia_f <- plot_grid(point.wise.DF.SS.Comb.plot_anemia_f, point.wise.DF.SS.Comb.diff.plot_anemia_f, ncol = 1, labels = c("B", "C"))


plot_composite_anemia_f <- plot_grid(plot_study_anemia_f, plot_comp_effect_anemia_f,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_anemia_f
