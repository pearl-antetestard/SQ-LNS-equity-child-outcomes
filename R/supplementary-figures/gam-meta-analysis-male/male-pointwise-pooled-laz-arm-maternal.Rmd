---
title: "Absolute treatment effect by Pointwise meta-analysis - laz, male"
...
df_analysis_laz_m <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 0) %>%
...
SS.Comb_laz_m <- df_analysis_laz_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_laz_m) {
    study_name <- unique(df_analysis_laz_m$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(laz_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_laz_m,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(laz_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_laz_m <- df_analysis_laz_m %>%
...
predictions.SS.Comb_laz_m <- new.data_laz_m %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_laz_m, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_mit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_laz_m)
...
plot_study_laz_m <- predictions.SS.Comb_laz_m %>%
...
point.wise.DF.SS.Comb_laz_m =  pointwise.ma(predictions.SS.Comb_laz_m,
...
point.wise.DF.SS.Comb_laz_m$RE.meta  =        point.wise.DF.SS.Comb_laz_m$RE.meta
point.wise.DF.SS.Comb_laz_m$RE.meta.upper  =  point.wise.DF.SS.Comb_laz_m$RE.meta.upper
point.wise.DF.SS.Comb_laz_m$RE.meta.lower  =  point.wise.DF.SS.Comb_laz_m$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_laz_m = point.wise.DF.SS.Comb_laz_m %>%
...
scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_laz_m$iwi),   max(point.wise.DF.SS.Comb_laz_m$iwi), by = 10))




print(point.wise.DF.SS.Comb.plot_laz_m)
...
predictions.SS.Comb_laz_m =predictions.SS.Comb_laz_m %>%
...
absolute_diff_SS.Comb_laz_m = risk.diff.creator(dataframe = predictions.SS.Comb_laz_m,
...
absolute_diff_SS.Comb_laz_m =  absolute_diff_SS.Comb_laz_m %>%
...
point.wise.absolute_diff_SS.Comb_laz_m  =  pointwise.ma(data = absolute_diff_SS.Comb_laz_m,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_laz_m =  point.wise.absolute_diff_SS.Comb_laz_m %>%
...
point.wise.DF.SS.Comb.diff.plot_laz_m =point.wise.absolute_diff_SS.Comb_laz_m %>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_laz_m$iwi),  
                                  max(point.wise.absolute_diff_SS.Comb_laz_m$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_laz_m)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_laz_m =point.wise.absolute_diff_SS.Comb_laz_m %>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_laz_m)

point.wise.pvalQ.plot_laz_m =point.wise.absolute_diff_SS.Comb_laz_m %>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_laz_m)


plot_composite_hetero_laz_m <- plot_grid(point.wise.Q.plot_laz_m, point.wise.pvalQ.plot_laz_m,
                               nrow=2, align = "h") 
plot_composite_hetero_laz_m
...
plot_comp_effect_laz_m <- plot_grid(point.wise.DF.SS.Comb.plot_laz_m, point.wise.DF.SS.Comb.diff.plot_laz_m, ncol = 1, labels = c("B", "C"))


plot_composite_laz_m <- plot_grid(plot_study_laz_m, plot_comp_effect_laz_m,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_laz_m
