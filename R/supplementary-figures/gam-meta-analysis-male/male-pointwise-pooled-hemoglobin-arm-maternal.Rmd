---
title: "Absolute treatment effect by Pointwise meta-analysis - hemoglobin concentration, male"
...
df_analysis_hgb_m <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 0) %>%
...
SS.Comb_hgb_m <- df_analysis_hgb_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_hgb_m) {
    study_name <- unique(df_analysis_hgb_m$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(b1_hb_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_hgb_m,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(b1_hb_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_hgb_m <- df_analysis_hgb_m %>%
...
predictions.SS.Comb_hgb_m <- new.data_hgb_m %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_hgb_m, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_mit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_hgb_m)
...
plot_study_hgb_m <- predictions.SS.Comb_hgb_m %>%
  ...
  ylab("Hemoglobin concentration (male)") +
  xlab("IWI") +
...
point.wise.DF.SS.Comb_hgb_m =  pointwise.ma(predictions.SS.Comb_hgb_m,
...
point.wise.DF.SS.Comb_hgb_m$RE.meta  =        point.wise.DF.SS.Comb_hgb_m$RE.meta
point.wise.DF.SS.Comb_hgb_m$RE.meta.upper  =  point.wise.DF.SS.Comb_hgb_m$RE.meta.upper
point.wise.DF.SS.Comb_hgb_m$RE.meta.lower  =  point.wise.DF.SS.Comb_hgb_m$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_hgb_m = point.wise.DF.SS.Comb_hgb_m %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled hemoglobin concentration (male)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_hgb_m$iwi),  max(point.wise.DF.SS.Comb_hgb_m$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_hgb_m)
...
predictions.SS.Comb_hgb_m=predictions.SS.Comb_hgb_m%>%
...
absolute_diff_SS.Comb_hgb_m = risk.diff.creator(dataframe = predictions.SS.Comb_hgb_m,
...
absolute_diff_SS.Comb_hgb_m =  absolute_diff_SS.Comb_hgb_m%>%
...
point.wise.absolute_diff_SS.Comb_hgb_m  =  pointwise.ma(data = absolute_diff_SS.Comb_hgb_m,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_hgb_m =  point.wise.absolute_diff_SS.Comb_hgb_m%>%
...
point.wise.DF.SS.Comb.diff.plot_hgb_m=point.wise.absolute_diff_SS.Comb_hgb_m%>%
  ...
  theme(axis.title.y = element_text(size = 18),
        ...
        axis.text.y = element_text(face="bold",  size=12),
        axis.text.x = element_text(face="bold",  size=12))  +
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_hgb_m$iwi),  max(point.wise.absolute_diff_SS.Comb_hgb_m$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_hgb_m)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_hgb_m=point.wise.absolute_diff_SS.Comb_hgb_m%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_hgb_m)

point.wise.pvalQ.plot_hgb_m=point.wise.absolute_diff_SS.Comb_hgb_m%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_hgb_m)


plot_composite_hetero_hgb_m <- plot_grid(point.wise.Q.plot_hgb_m, point.wise.pvalQ.plot_hgb_m,
                               nrow=2, align = "h") 
plot_composite_hetero_hgb_m
...
plot_comp_effect_hgb_m <- plot_grid(point.wise.DF.SS.Comb.plot_hgb_m, point.wise.DF.SS.Comb.diff.plot_hgb_m, ncol = 1, labels = c("B", "C"))


plot_composite_hgb_m <- plot_grid(plot_study_hgb_m, plot_comp_effect_hgb_m,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_hgb_m
