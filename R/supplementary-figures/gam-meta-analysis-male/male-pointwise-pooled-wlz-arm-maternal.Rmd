---
title: "Absolute treatment effect by Pointwise meta-analysis - wlz, male"
...
df_analysis_wlz_m <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 0) %>%
  dplyr::mutate(study = sub("s[0-9]*_", "", study.x)) %>%
  select(study, pid, region, cluster, arms_maternal, iwi, wlz_el) %>% 
  dplyr::filter(!is.na(iwi)) %>%
  dplyr::filter(!is.na(arms_maternal)) %>%
  dplyr::filter(!is.na(wlz_el)) %>%
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(wlz_el ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb_wlz_m <- df_analysis_wlz_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wlz_m) {
    study_name <- unique(df_analysis_wlz_m$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(wlz_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_wlz_m,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(wlz_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_wlz_m <- df_analysis_wlz_m %>%
...
predictions.SS.Comb_wlz_m <- new.data_wlz_m %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_wlz_m, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_mit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_wlz_m)
...
plot_study_wlz_m <- predictions.SS.Comb_wlz_m %>%
...
point.wise.DF.SS.Comb_wlz_m =  pointwise.ma(predictions.SS.Comb_wlz_m,
...
point.wise.DF.SS.Comb_wlz_m$RE.meta  =        point.wise.DF.SS.Comb_wlz_m$RE.meta
point.wise.DF.SS.Comb_wlz_m$RE.meta.upper  =  point.wise.DF.SS.Comb_wlz_m$RE.meta.upper
point.wise.DF.SS.Comb_wlz_m$RE.meta.lower  =  point.wise.DF.SS.Comb_wlz_m$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_wlz_m = point.wise.DF.SS.Comb_wlz_m %>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_wlz_m$iwi),  max(point.wise.DF.SS.Comb_wlz_m$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_wlz_m)
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_wlz.png"),
...
predictions.SS.Comb_wlz_m =predictions.SS.Comb_wlz_m %>%
...
absolute_diff_SS.Comb_wlz_m = risk.diff.creator(dataframe = predictions.SS.Comb_wlz_m,
...
absolute_diff_SS.Comb_wlz_m =  absolute_diff_SS.Comb_wlz_m %>%
...
point.wise.absolute_diff_SS.Comb_wlz_m  =  pointwise.ma(data = absolute_diff_SS.Comb_wlz_m,
                                                  clustering.variable = "study",
                                                  combining.variables = c("iwi"),
                                                  predicted.outcome = "fit.diff",
                                                  predicted.outcome.se = NULL,
                                                  predicted.outcome.CI = c("diff.lower","diff.upper"),
                                                  tau.method = "REML" # Used DL because the model did not converge with REML
                                                  )

point.wise.absolute_diff_SS.Comb_wlz_m =  point.wise.absolute_diff_SS.Comb_wlz_m %>%
...
point.wise.DF.SS.Comb.diff.plot_wlz_m =point.wise.absolute_diff_SS.Comb_wlz_m %>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_wlz_m$iwi),  max(point.wise.absolute_diff_SS.Comb_wlz_m$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_wlz_m)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_wlz_m=point.wise.absolute_diff_SS.Comb_wlz_m %>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_wlz_m)

point.wise.pvalQ.plot_wlz_m=point.wise.absolute_diff_SS.Comb_wlz_m%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_wlz_m)


plot_composite_hetero_wlz_m <- plot_grid(point.wise.Q.plot_wlz_m, point.wise.pvalQ.plot_wlz_m,
                               nrow=2, align = "h") 
plot_composite_hetero_wlz_m
...
# Save the plot
#ggsave(here::here("output", "qplot_pooled_pointwise_wlz.png"),
...
plot_comp_effect_wlz_m <- plot_grid(point.wise.DF.SS.Comb.plot_wlz_m, point.wise.DF.SS.Comb.diff.plot_wlz_m, ncol = 1, labels = c("B", "C"))


plot_composite_wlz_m <- plot_grid(plot_study_wlz_m, plot_comp_effect_wlz_m,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_wlz_m
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_wlz_arm_maternal.png"),
#       plot = plot_composite_wlz, width = 14, height = 16, dpi = 300)
```
