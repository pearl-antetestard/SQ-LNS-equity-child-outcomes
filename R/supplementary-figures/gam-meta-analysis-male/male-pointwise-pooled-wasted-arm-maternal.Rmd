...
df_analysis_wasted_m <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 0) %>%
...
table(df_analysis_wasted_m$study, df_analysis_wasted_m$wasted_el)
...
df_analysis_wasted_m <- subset(df_analysis_wasted_m, !(study %in% c("HAITI", "PROMISM")))
...
SS.Comb_wasted_m <- df_analysis_wasted_m %>%
  dplyr::mutate(study = factor(study)) %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wasted_m) {
    study_name <- unique(df_analysis_wasted_m$study)
    if (study_name %in% c("DOSE", "GHANA","DYADG","DYADM")) {
      model <- gam(wasted_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_wasted_m,
...
new.data_wasted_m <- df_analysis_wasted_m %>%
...
predictions.SS.Comb_wasted_m <- new.data_wasted_m %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_wasted_m, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_mit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_wasted_m)
...
plot_study_wasted_m <- predictions.SS.Comb_wasted_m %>%
  ...
  ylab("Probability of wasting (male)") +
  xlab("IWI") +
...
point.wise.DF.SS.Comb_wasted_m =  pointwise.ma(predictions.SS.Comb_wasted_m,
...
point.wise.DF.SS.Comb_wasted_m$RE.meta  =        expit(point.wise.DF.SS.Comb_wasted_m$RE.meta)
point.wise.DF.SS.Comb_wasted_m$RE.meta.upper  =  expit(point.wise.DF.SS.Comb_wasted_m$RE.meta.upper)
point.wise.DF.SS.Comb_wasted_m$RE.meta.lower  =  expit(point.wise.DF.SS.Comb_wasted_m$RE.meta.lower)
...
point.wise.DF.SS.Comb.plot_wasted_m = point.wise.DF.SS.Comb_wasted_m %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled probability of wasting (male)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_wasted_m$iwi), max(point.wise.DF.SS.Comb_wasted_m$iwi), by = 10))



print(point.wise.DF.SS.Comb.plot_wasted_m)
...
predictions.SS.Comb_wasted_m =predictions.SS.Comb_wasted_m %>%
...
absolute_diff_SS.Comb_wasted_m = risk.diff.creator(dataframe = predictions.SS.Comb_wasted_m,
...
absolute_diff_SS.Comb_wasted_m =  absolute_diff_SS.Comb_wasted_m %>%
...
point.wise.absolute_diff_SS.Comb_wasted_m  =  pointwise.ma(data = absolute_diff_SS.Comb_wasted_m,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_wasted_m =  point.wise.absolute_diff_SS.Comb_wasted_m %>%
...
point.wise.DF.SS.Comb.diff.plot_wasted_m =point.wise.absolute_diff_SS.Comb_wasted_m %>%
  ...
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_wasted_m$iwi), max(point.wise.absolute_diff_SS.Comb_wasted_m$iwi), by = 10))

print(point.wise.DF.SS.Comb.diff.plot_wasted_m)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_wasted_m =point.wise.absolute_diff_SS.Comb_wasted_m %>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_wasted_m)

point.wise.pvalQ.plot_wasted_m = point.wise.absolute_diff_SS.Comb_wasted_m %>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_wasted_m)


plot_composite_hetero_wasted_m <- plot_grid(point.wise.Q.plot_wasted_m, point.wise.pvalQ.plot_wasted_m,
                               nrow=2, align = "h") 
plot_composite_hetero_wasted_m
...
plot_comp_effect_wasted_m <- plot_grid(point.wise.DF.SS.Comb.plot_wasted_m, point.wise.DF.SS.Comb.diff.plot_wasted_m, ncol = 1, labels = c("B", "C"))


plot_composite_wasted_m <- plot_grid(plot_study_wasted_m, plot_comp_effect_wasted_m,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_wasted_m
