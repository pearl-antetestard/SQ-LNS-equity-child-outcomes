---
title: "Absolute treatment effect by Pointwise meta-analysis - executive function score, male"
...
df_analysis_exec_m <- readRDS(file = here::here("data", "1-final",
                      "df_analysis_iwi.rds")) %>% 
  subset(female == 0) %>%
...
#SS.Comb <- df_analysis %>%
 ...
 #     model <- gam(d12_execfun_el ~ arm_primary + iwi * arm_primary + s(iwi, bs = "cr", 
  #                                         by = arm_primary),
  #                data = df_analysis,
...
SS.Comb_exec_m <- df_analysis_exec_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_exec_m) {
    study_name <- unique(df_analysis_exec_m$study)
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- gam(d12_execfun_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal),
                  data = df_analysis_exec_m,
                  family = "gaussian", method = "REML")
    } else {
      model <- gam(d12_execfun_el ~ arms_maternal + iwi * arms_maternal + s(iwi, bs = "cr", 
                                           by = arms_maternal) +
...
new.data_exec_m <- df_analysis_exec_m %>%
...
predictions.SS.Comb_exec_m <- new.data_exec_m %>%
  droplevels() %>%
  arrange(desc(study)) %>%
  group_by(study) %>%
  nest() %>%
  full_join(SS.Comb_exec_m, by = "study") %>%
  group_by(study)%>% 
  do(augment(.$model[[1]], newdata = .$data[[1]], se_mit =T)) 

  
# View the resulting dataframe
print(predictions.SS.Comb_exec_m)
...
plot_study_exec_m <- predictions.SS.Comb_exec_m%>%
  ggplot(., aes(iwi,.fitted, color= arms_maternal))+ 
  geom_line(size=1.25)+ 
  facet_wrap(.~study, ncol = 2, nrow = 3) + 
  ...
  ylab("Executive function score (male)") +
  xlab("IWI") +
...
point.wise.DF.SS.Comb_exec_m =  pointwise.ma(predictions.SS.Comb_exec_m,
...
point.wise.DF.SS.Comb_exec_m$RE.meta  =        
  point.wise.DF.SS.Comb_exec_m$RE.meta
point.wise.DF.SS.Comb_exec_m$RE.meta.upper  =  point.wise.DF.SS.Comb_exec_m$RE.meta.upper
point.wise.DF.SS.Comb_exec_m$RE.meta.lower  =  point.wise.DF.SS.Comb_exec_m$RE.meta.lower
...
point.wise.DF.SS.Comb.plot_exec_m = point.wise.DF.SS.Comb_exec_m%>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled executive function score (male)')) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = seq(min(point.wise.DF.SS.Comb_exec_m$iwi),  max(point.wise.DF.SS.Comb_exec_m$iwi), by = 10))


print(point.wise.DF.SS.Comb.plot_exec_m)
...
predictions.SS.Comb_exec_m=predictions.SS.Comb_exec_m%>%
...
absolute_diff_SS.Comb_exec_m = risk.diff.creator(dataframe = predictions.SS.Comb_exec_m,
...
absolute_diff_SS.Comb_exec_m=  absolute_diff_SS.Comb_exec_m%>%
...
point.wise.absolute_diff_SS.Comb_exec_m  =  pointwise.ma(data = absolute_diff_SS.Comb_exec_m,
                                                  clustering.variable = "study",
...
point.wise.absolute_diff_SS.Comb_exec_m =  point.wise.absolute_diff_SS.Comb_exec_m%>%
...
point.wise.DF.SS.Comb.diff.plot_exec_m=point.wise.absolute_diff_SS.Comb_exec_m%>%
  ...
  theme(axis.title.y = element_text(size = 18),
        ...
        strip.text = element_text(face="bold", size=14, hjust = 0.5),
        axis.text.y = element_text(face="bold",  size=12),
        axis.text.x = element_text(face="bold",  size=12)) + ylim(c(-0.4,0.3)) +
  scale_x_continuous(breaks = seq(min(point.wise.absolute_diff_SS.Comb_exec_m$iwi),  max(point.wise.absolute_diff_SS.Comb_exec_m$iwi), by = 10))


print(point.wise.DF.SS.Comb.diff.plot_exec_m)

```

```{r 11, echo = TRUE, warning = FALSE}

point.wise.Q.plot_exec_m=point.wise.absolute_diff_SS.Comb_exec_m%>%
  ggplot(aes(x = iwi,y=Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.Q.plot_exec_m)

point.wise.pvalQ.plot_exec_m=point.wise.absolute_diff_SS.Comb_exec_m%>%
  ggplot(aes(x = iwi,y=pval.Q)) + geom_line(size=1)+
  xlab(bquote('IWI')) +
  theme_minimal()

print(point.wise.pvalQ.plot_exec_m)


plot_composite_hetero_exec_m <- plot_grid(point.wise.Q.plot_exec_m, point.wise.pvalQ.plot_exec_m,
                               nrow=2, align = "h") 
plot_composite_hetero_exec_m
...
plot_comp_effect_exec_m <- plot_grid(point.wise.DF.SS.Comb.plot_exec_m, point.wise.DF.SS.Comb.diff.plot_exec_m, ncol = 1, labels = c("B", "C"))


plot_composite_exec_m <- plot_grid(plot_study_exec_m, plot_comp_effect_exec_m,
                               ncol=2, labels = c("A", "", "")) 

plot_composite_exec_m
...
# Save the plot
#ggsave(here::here("output", "gam_pooled_pointwise_d12_execfun_el_arm_maternal.png"),
   #    plot = plot_composite_exec, width = 14, height = 16, dpi = 300)
```
