---
title: "Measuring inequalities: RII and SII - severe stunting, female"
...
ecdf_iwi_decile <- ecdf(df_analysis_severe_stunted_f$iwi_decile)

# Apply the ECDF to get the relative rank
df_analysis_severe_stunted_f <- df_analysis_severe_stunted_f %>%
  mutate(iwi_relative_rank = ecdf_iwi_decile(iwi_decile))

# Print the data frame with the new relative rank column
print(df_analysis_severe_stunted_f)
...
ineq_f <- df_analysis_severe_stunted_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_severe_stunted_f) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_severe_stunted_f %>% filter(arms_maternal == "Control")
      ...
      # Intervention group
      df_interv <- df_analysis_severe_stunted_f %>% filter(arms_maternal == "SQ-LNS")
...
prevalence_values_severe_stunted <- df_analysis_severe_stunted_f %>%
...
overall_prevalence_severe_stunted <- df_analysis_severe_stunted_f %>%
  group_by(study) %>%
  summarise(
...
meta_rii_severe_stunted_f <- ggplot(aes(x = arm, y = RE.meta), 
                                  data = meta_results_rii_f) +
  geom_point(aes(color = arm), size = 2.5) +
  geom_errorbar(aes(x = arm, ymin = RE.meta.lower, ymax = RE.meta.upper), width = 0.1, data = meta_results_rii_f) +
...
meta_rii_severe_stunted_f

```

```{r plot sii}

meta_sii_severe_stunted_f <- ggplot(meta_results_sii_f, aes(x = arm, y = RE.meta*100)) +
  geom_point(aes(color = arm), size = 2.5) +
...
meta_sii_severe_stunted_f

```

```{r combine}

meta_plot_severe_stunted_f <- plot_grid(meta_rii_severe_stunted_f, meta_sii_severe_stunted_f, ncol = 1, nrow = 2)

meta_plot_severe_stunted_f
```

## Save the results
```{r save results}

write.csv2(ineq_f, file = here::here("output", "rii_sii_severe_stunted_f.csv"), row.names = FALSE)
