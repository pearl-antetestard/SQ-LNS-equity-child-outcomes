---
title: "Measuring inequalities: RII and SII - anemia, female"
...
# Load the data
df_analysis_anemia_f <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  filter(female == 1) %>%
...
ineq_f <- df_analysis_anemia_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_anemia_f) {
    study_name <- unique(df_analysis_anemia_f$study)
    results <- list()
    
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_anemia_f %>% filter(arms_maternal == "Control")
      ...
      # Intervention group
      df_interv <- df_analysis_anemia_f %>% filter(arms_maternal == "SQ-LNS")
...
prevalence_values_anemia <- df_analysis_anemia_f %>%
...
overall_prevalence_anemia <- df_analysis_anemia_f %>%
  group_by(study) %>%
  summarise(
...
df <- left_join(ineq_f, df_sqlns, by = "study") 
...
meta_results_rii_f <- rbind(RII_control_df_wide, RII_intervention_df_wide)
...
meta_results_rii_f_prev <- cbind(meta_results_rii_f, meta_results_prevalence)


meta_results_sii_f <- rbind(SII_control_df_wide, SII_intervention_df_wide)
meta_results_sii_f_prev <- cbind(meta_results_sii_f, meta_results_prevalence)

```

```{r plot rii}

meta_rii_anemia_f <-  ggplot(meta_results_rii_f, aes(x = arm, y = RE.meta)) +
...
meta_sii_anemia_f <- ggplot(meta_results_sii_f, aes(x = arm, y = RE.meta*100)) +
  geom_point(aes(color = arm), size = 2.5) +
...
meta_plot_anemia_f <- plot_grid(meta_rii_anemia_f, meta_sii_anemia_f, ncol = 1, nrow = 2)
...
write.csv2(ineq_f, file = here::here("output", "rii_sii_anemia_f.csv"), row.names = FALSE)
