---
title: "Measuring inequalities: RII and SII - stunted, female"
...
# Load the data
df_analysis_stunted_f <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  filter(female == 1) %>%
...
ecdf_iwi_decile <- ecdf(df_analysis_stunted_f$iwi_decile)
...
ineq_f <- df_analysis_stunted_f %>%
  ...
  map_dfr(function(df_analysis_stunted_f) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_stunted_f %>% filter(arms_maternal == "Control")
      ...
      # Intervention group
      df_interv <- df_analysis_stunted_f %>% filter(arms_maternal == "SQ-LNS")
...
prevalence_values_stunted <- df_analysis_stunted_f %>%
...
overall_prevalence_stunted <- df_analysis_stunted_f %>%
  group_by(study) %>%
  summarise(
...
meta_rii_stunted_f <- ggplot(aes(x = arm, y = RE.meta), data = meta_results_rii) +
...
meta_sii_stunted_f <- ggplot(meta_results_sii, aes(x = arm, y = RE.meta*100)) +
  geom_point(aes(color = arm), size = 2.5) +
...
meta_plot_stunted_f <- plot_grid(meta_rii_stunted_f, meta_sii_stunted_f, ncol = 1, nrow = 2)
...
write.csv2(ineq_f, file = here::here("output", "rii_sii_stunted_f.csv"), row.names = FALSE)
