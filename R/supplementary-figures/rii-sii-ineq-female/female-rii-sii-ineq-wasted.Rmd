---
title: "Measuring ineq_fualities: RII and SII - wasted, female"
...
# Load the data
df_analysis_wasted_f <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  filter(female == 1) %>%
...
RII = 1 or SII = 0: no ineq_fuality

```{r RII and SII}

ineq_f <- df_analysis_wasted_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wasted_f) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_wasted_f %>% filter(arms_maternal == "Control")
      ...
      # Intervention group
      df_interv <- df_analysis_wasted_f %>% filter(arms_maternal == "SQ-LNS")
...
prevalence_values_wasted <- df_analysis_wasted_f %>%
...
overall_prevalence_wasted <- df_analysis_wasted_f %>%
  group_by(study) %>%
  summarise(
...
meta_rii_wasted_f <- ggplot(aes(x = arm, y = RE.meta), data = meta_results_rii_f) +
...
meta_sii_wasted_f <- ggplot(meta_results_sii_f, aes(x = arm, y = RE.meta*100)) +
   geom_point(aes(color = arm), size = 2.5) +
...
meta_plot_wasted_f <- plot_grid(meta_rii_wasted_f, meta_sii_wasted_f, ncol = 1, nrow = 2)
...
write.csv2(ineq_f, file = here::here("output", "rii_sii_wasted_f.csv"), row.names = FALSE)
