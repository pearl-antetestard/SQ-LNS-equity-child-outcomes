---
title: "Linear regression - interaction between treatment and maternal education - executive function"
...
df_analysis_exec <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  dplyr::mutate(study = sub("s[0-9]*_", "", study.x)) %>%
  select(study, pid, region, cluster, arms_maternal, raw_mateducat, d12_execfun_el, iwi) %>%
...
head(df_analysis_exec)

table(df_analysis_exec$study, df_analysis_exec$mateduc_cat)
...
model_noint <- lm_robust(d12_execfun_el ~ mateduc_cat + arms_maternal, data = df_analysis_exec,
                         clusters = study)

# Fit the more complex model with interaction
model_int <- lm_robust(d12_execfun_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, data = df_analysis_exec, clusters = study)
...
inter <- df_analysis_exec %>%
  ...
  map_dfr(function(df_analysis_exec) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(d12_execfun_el ~ mateduc_cat + arms_maternal + 
      ...
      model <- lm_robust(d12_execfun_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, 
                         data = df_analysis_exec, clusters = cluster)
...
forest_meta_primary_incSecondary <- forest(meta1_primary_incSecondary, 
       layout = "JAMA", 
       leftlabs = c("Study", "d12_execfun_el (primary/inc.Secondary:arms_maternalSQ-LNS) [95% CI]"),
...
forest_meta_secondary_greater <- forest(meta1_secondary_greater, 
       layout = "JAMA", 
       leftlabs = c("Study", "d12_execfun_el (secondary/greater:arms_maternalSQ-LNS) [95% CI]"),
       label.right = "Favorable", 
       label.left = "Unfavorable",
       digits = 3,
       prediction.subgroup = inter$arms_maternal)
