---
title: "Linear regression - interaction between treatment and maternal education - wlz"
...
df_analysis_wlz <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  ...
  select(study, pid, region, cluster, arms_maternal, raw_mateducat, wlz_el, iwi) %>%
...
head(df_analysis_wlz)

table(df_analysis_wlz$study, df_analysis_wlz$mateduc_cat)
...
model_noint <- lm_robust(wlz_el ~ mateduc_cat + arms_maternal, data = df_analysis_wlz,
                         clusters = study)

# Fit the more complex model with interaction
model_int <- lm_robust(wlz_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, data = df_analysis_wlz, clusters = study)
...
inter <- df_analysis_wlz %>%
  ...
  map_dfr(function(df_analysis_wlz) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(wlz_el ~ mateduc_cat + arms_maternal + 
      ...
      model <- lm_robust(wlz_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, 
                         data = df_analysis_wlz, clusters = cluster)
...
forest_meta_primary_incSecondary <- forest(meta1_primary_incSecondary, 
       layout = "JAMA", 
       leftlabs = c("Study", "wlz_el (primary/inc.Secondary:arms_maternalSQ-LNS) [95% CI]"),
...
forest_meta_secondary_greater <- forest(meta1_secondary_greater, 
       layout = "JAMA", 
       leftlabs = c("Study", "wlz_el (secondary/greater:arms_maternalSQ-LNS) [95% CI]"),
       label.right = "Favorable", 
       label.left = "Unfavorable",
       digits = 3,
       prediction.subgroup = inter$arms_maternal)
