---
title: "Linear regression - interaction between treatment and maternal education - language score"
...
df_analysis_language <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  dplyr::mutate(study = sub("s[0-9]*_", "", study.x)) %>%
  select(study, pid, region, cluster, arms_maternal, raw_mateducat, d1_lang_el, iwi) %>%
...
head(df_analysis_language)

table(df_analysis_language$study, df_analysis_language$mateduc_cat)
...
model_noint <- lm_robust(d1_lang_el ~ mateduc_cat + arms_maternal, data = df_analysis_language,
                         clusters = study)

# Fit the more complex model with interaction
model_int <- lm_robust(d1_lang_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, data = df_analysis_language, clusters = study)
...
inter <- df_analysis_language %>%
  ...
  map_dfr(function(df_analysis_language) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(d1_lang_el ~ mateduc_cat + arms_maternal + 
      ...
      model <- lm_robust(d1_lang_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, 
                         data = df_analysis_language, clusters = cluster)
...
forest_meta_primary_incSecondary <- forest(meta1_primary_incSecondary, 
       layout = "JAMA", 
       leftlabs = c("Study", "d1_lang_el (primary/inc.Secondary:arms_maternalSQ-LNS) [95% CI]"),
...
forest_meta_secondary_greater <- forest(meta1_secondary_greater, 
       layout = "JAMA", 
       leftlabs = c("Study", "d1_lang_el (secondary/greater:arms_maternalSQ-LNS) [95% CI]"),
       label.right = "Favorable", 
       label.left = "Unfavorable",
       digits = 3,
       prediction.subgroup = inter$arms_maternal)
