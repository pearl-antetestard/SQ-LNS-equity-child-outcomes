---
title: "Linear regression - interaction between treatment and maternal education - anemia"
...
df_analysis_anemia <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  ...
  select(study, pid, region, cluster, arms_maternal, raw_mateducat, b2_anemia_el, iwi) %>%
...
head(df_analysis_anemia)

table(df_analysis_anemia$study, df_analysis_anemia$mateduc_cat)
...
model_noint <- lm_robust(b2_anemia_el ~ mateduc_cat + arms_maternal, data = df_analysis_anemia,
                         clusters = study)

# Fit the more complex model with interaction
model_int <- lm_robust(b2_anemia_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, data = df_analysis_anemia, clusters = study)
...
inter <- df_analysis_anemia %>%
  ...
  map_dfr(function(df_analysis_anemia) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(b2_anemia_el ~ mateduc_cat + arms_maternal + 
      ...
      model <- lm_robust(b2_anemia_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, 
...
forest_meta_primary_incSecondary <- forest(meta1_primary_incSecondary, 
       layout = "JAMA", 
       leftlabs = c("Study", "b2_anemia_el (primary/inc.Secondary:arms_maternalSQ-LNS) [95% CI]"),
...
forest_meta_secondary_greater <- forest(meta1_secondary_greater, 
       layout = "JAMA", 
       leftlabs = c("Study", "b2_anemia_el (secondary/greater:arms_maternalSQ-LNS) [95% CI]"),
       label.right = "Favorable", 
       label.left = "Unfavorable",
       digits = 3,
       prediction.subgroup = inter$arms_maternal)
