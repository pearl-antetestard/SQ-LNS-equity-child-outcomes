---
title: "Linear regression - interaction between treatment and maternal education - socioemotional score"
...
df_analysis_socioemo <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  dplyr::mutate(study = sub("s[0-9]*_", "", study.x)) %>%
  select(study, pid, region, cluster, arms_maternal, raw_mateducat, d5_socioemo_el, iwi) %>%
...
head(df_analysis_socioemo)

table(df_analysis_socioemo$study, df_analysis_socioemo$mateduc_cat)
...
model_noint <- lm_robust(d5_socioemo_el ~ mateduc_cat + arms_maternal, data = df_analysis_socioemo,
                         clusters = study)

# Fit the more complex model with interaction
model_int <- lm_robust(d5_socioemo_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, data = df_analysis_socioemo, clusters = study)
...
inter <- df_analysis_socioemo %>%
  ...
  map_dfr(function(df_analysis_socioemo) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(d5_socioemo_el ~ mateduc_cat + arms_maternal + 
      ...
      model <- lm_robust(d5_socioemo_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, 
                         data = df_analysis_socioemo, clusters = cluster)
...
forest_meta_primary_incSecondary <- forest(meta1_primary_incSecondary, 
       layout = "JAMA", 
       leftlabs = c("Study", "d5_socioemo_el (primary/inc.Secondary:arms_maternalSQ-LNS) [95% CI]"),
...
forest_meta_secondary_greater <- forest(meta1_secondary_greater, 
       layout = "JAMA", 
       leftlabs = c("Study", "d5_socioemo_el (secondary/greater:arms_maternalSQ-LNS) [95% CI]"),
       label.right = "Favorable", 
       label.left = "Unfavorable",
       digits = 3,
       prediction.subgroup = inter$arms_maternal)
