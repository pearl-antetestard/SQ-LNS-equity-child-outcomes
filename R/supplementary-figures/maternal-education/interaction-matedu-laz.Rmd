---
title: "Linear regression - interaction between treatment and maternal education - laz"
...
table(df_analysis_laz$study, df_analysis_laz$mateduc_cat)
...
model_noint <- lm_robust(laz_el ~ mateduc_cat + arms_maternal, data = df_analysis_laz,
                         clusters = study)

# Fit the more complex model with interaction
model_int <- lm_robust(laz_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, data = df_analysis_laz, clusters = study)
...
```{r interaction per study, echo=TRUE, warning=FALSE}
...
inter <- df_analysis_laz %>%
  ...
  map_dfr(function(df_analysis_laz) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(laz_el ~ mateduc_cat + arms_maternal + 
      ...
      model <- lm_robust(laz_el ~ mateduc_cat + arms_maternal + mateduc_cat * arms_maternal, 
...
forest_meta_primary_incSecondary <- forest(meta1_primary_incSecondary, 
       layout = "JAMA", 
       leftlabs = c("Study", "laz_el (primary/inc.Secondary:arms_maternalSQ-LNS) [95% CI]"),
...
forest_meta_secondary_greater <- forest(meta1_secondary_greater, 
       layout = "JAMA", 
       leftlabs = c("Study", "laz_el (secondary/greater:arms_maternalSQ-LNS) [95% CI]"),
       label.right = "Favorable", 
       label.left = "Unfavorable",
       digits = 3,
       prediction.subgroup = inter$arms_maternal)
