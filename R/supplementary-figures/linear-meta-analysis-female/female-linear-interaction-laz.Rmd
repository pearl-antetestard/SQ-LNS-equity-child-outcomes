---
title: "Linear regression - interaction between treatment and IWI - laz, female"
...
inter_f <- df_analysis_laz_f %>%
  ...
  map_dfr(function(df_analysis_laz_f) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(laz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_laz_f, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_laz_f$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_f <- df_analysis_laz_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_laz_f) {
    study_name <- unique(df_analysis_laz_f$study)
...
study_list <- unique(df_analysis_laz_f$study)
results_f <- lapply(study_list, function(study) {
  df_study_f <- df_analysis_laz_f %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_laz_f$iwi), by = 1, length.out = 71)
