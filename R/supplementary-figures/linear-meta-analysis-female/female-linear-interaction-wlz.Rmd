---
title: "Linear regression - interaction between treatment and IWI - wlz, female"
...
inter_f <- df_analysis_wlz_f %>%
  ...
  map_dfr(function(df_analysis_wlz_f) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(wlz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_wlz_f, clusters = cluster)
...
laz_eff_f <- c(meta1_f$TE.random, meta1_f$upper.random, meta1_f$lower.random, 
              meta1_f$seTE.random, meta1_f$Q, meta1_f$df.Q, meta1_f$I2,
              meta1_f$tau2,
              meta1_f$pval.random)
...
pooled_slope_plot_f <- ggplot(aes(x = term, y = RE.meta), 
                              data = laz_eff_df_wide_f) +
  geom_point(size = 3) +
...
iwi_values <- seq(min(df_analysis_wlz_f$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_f <- df_analysis_wlz_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wlz_f) {
    study_name <- unique(df_analysis_wlz_f$study)
...
study_list <- unique(df_analysis_wlz_f$study)
results_f <- lapply(study_list, function(study) {
  df_study_f <- df_analysis_wlz_f %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_wlz_f$iwi), by = 1, length.out = 71)
