...
table(df_analysis_stunted_f$study, df_analysis_stunted_f$stunted_el)
...
df_analysis_stunted_f <- df_analysis_stunted_f %>%
  filter(study != "GHANA")
...
inter_f <- df_analysis_stunted_f %>%
  ...
  map_dfr(function(df_analysis_stunted_f) {
    ...
    if (study_name %in% c("DOSE","HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(stunted_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_stunted_f, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_stunted_f$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_f <- df_analysis_stunted_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_stunted_f) {
    study_name <- unique(df_analysis_stunted_f$study)
...
study_list <- unique(df_analysis_stunted_f$study)
results_f <- lapply(study_list, function(study) {
  df_study_f <- df_analysis_stunted_f %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_stunted_f$iwi), by = 1, length.out = 71)
