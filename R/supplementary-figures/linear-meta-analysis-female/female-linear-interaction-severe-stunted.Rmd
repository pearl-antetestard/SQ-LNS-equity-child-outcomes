---
title: "Linear regression - interaction between treatment and IWI - severe stunting, female"
...
df_analysis_severe_stunted_f <- readRDS(file = here::here("data", "1-final",
  ...
  dplyr::mutate(arms_maternal = factor(arms_maternal, levels = c(0,1), 
                ...
                severe_stunted = case_when(laz_el < -3 ~ 1, 
                                           laz_el >= -3 ~ 0),
                severe_stunted = as.numeric(severe_stunted),
         iwi = as.numeric(iwi),
...
inter_f <- df_analysis_severe_stunted_f %>%
  ...
  map_dfr(function(df_analysis_severe_stunted_f) {
    ...
    if (study_name %in% c("DOSE")) {
      model <- lm_robust(severe_stunted ~ iwi + arms_maternal + iwi * arms_maternal, 
      ...
      model <- lm_robust(severe_stunted ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_severe_stunted_f, clusters = cluster)
...
ggplot(inter_f, aes(x = study, y = inter_treat_iwi)) +
  ...
  labs(x = "Study", y = "severe_stunted (arms_maternalSQ-LNS:iwi)") +
...
forest_meta_f <- forest(meta1_f, 
       layout = "JAMA", 
       leftlabs = c("Study", "severe_stunted (iwi:arms_maternalSQ-LNS) [95% CI]"),
...
laz_eff_df_wide_f$RE.meta  =        expit(laz_eff_df_wide_f$RE.meta)
laz_eff_df_wide_f$RE.meta.upper  =  expit(laz_eff_df_wide_f$RE.meta.upper)
laz_eff_df_wide_f$RE.meta.lower  =  expit(laz_eff_df_wide_f$RE.meta.lower)
...
pooled_slope_plot_f <- ggplot(aes(x = term, y = RE.meta), data = laz_eff_df_wide_f) +
  ...
  labs(x = "Random-effect meta-analysis", y = "Pooled probability of severe stunting (iwi:arms_maternalSQ-LNS)") +
  theme_minimal()  +
...
iwi_values <- seq(min(df_analysis_severe_stunted_f$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_f <- df_analysis_severe_stunted_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_severe_stunted_f) {
    study_name <- unique(df_analysis_severe_stunted_f$study)
...
plots_f <- predictions_f %>%
  ...
  scale_fill_manual(values = c('darkgrey','darkblue')) + 
      ...
      labs(title = paste("Interaction (iwi:arms_maternalSQ-LNS) -", unique(data$study)),
           x = "IWI",
           y = "Predicted probability of severe stunting",
           color = "Treatment",
           fill = "Treatment") +
...
## Backtransform predicted outcomes and their corresponding confidence intervals
point.wise.DF_f$RE.meta  =        expit(point.wise.DF_f$RE.meta)
point.wise.DF_f$RE.meta.upper  =  expit(point.wise.DF_f$RE.meta.upper)
point.wise.DF_f$RE.meta.lower  =  expit(point.wise.DF_f$RE.meta.lower)
  ...
  theme(axis.title.y = element_text(size = 18),
      ...
      annotate("text", x = min(point.wise.absolute_diff_f$iwi) * 0.8, 
               ...
               label = paste("interaction p-value:", 
                             round(as.numeric(unique(laz_eff_df_wide_f$pval.random)), 3)), 
               hjust = 0, vjust = 1, size = 3, color = "black")
...
study_list <- unique(df_analysis_severe_stunted_f$study)
results_f <- lapply(study_list, function(study) {
  df_study_f <- df_analysis_severe_stunted_f %>% filter(study == !!study)
  
  if (study %in% c("DOSE")) {
    model_study <- lm_robust(severe_stunted ~ iwi + arms_maternal + iwi * arms_maternal, 
                             data = df_study_f)
  } else {
    model_study <- lm_robust(severe_stunted ~ iwi + arms_maternal + iwi * arms_maternal, 
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_severe_stunted_f$iwi), by = 1, length.out = 71)
...
plot_diff_f %>%
  ...
  annotate(
    "text", 
    x = min(point.wise.absolute_diff_f$iwi) * 0.8, 
    y = max(point.wise.absolute_diff_f$RE.meta.upper), 
