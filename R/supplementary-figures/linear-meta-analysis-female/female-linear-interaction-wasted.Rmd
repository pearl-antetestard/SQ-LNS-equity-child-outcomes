---
title: "Linear regression - interaction between treatment and IWI - stunted, female"
...
# removed HAITI and PROMISM because of very small sample sizes for wasting
...
inter_f <- df_analysis_wasted_f %>%
  ...
  map_dfr(function(df_analysis_wasted_f) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(wasted_el ~ iwi + arms_maternal + iwi * arms_maternal, 
      ...
      model <- lm_robust(wasted_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_wasted_f, clusters = cluster)
...
pooled_slope_plot_f <- ggplot(aes(x = term, y = RE.meta), data = laz_eff_df_wide_f) +
  ...
  labs(x = "Random-effect meta-analysis", y = "Pooled wasted_el (iwi:arms_maternalSQ-LNS)") +
  theme_minimal()  +
...
iwi_values <- seq(min(df_analysis_wasted_f$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_f <- df_analysis_wasted_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wasted_f) {
    study_name <- unique(df_analysis_wasted_f$study)
...
point.wise.diff.plot_f = point.wise.absolute_diff_f %>%
  ...
  theme(axis.title.y = element_text(size = 18),
      ...
      annotate("text", x = min(point.wise.absolute_diff_f$iwi) * 0.8, 
               y = max(point.wise.absolute_diff_f$RE.meta.upper) + 0.1, 
...
study_list <- unique(df_analysis_wasted_f$study)
results_f <- lapply(study_list, function(study) {
  df_study_f <- df_analysis_wasted_f %>% filter(study == !!study)
  
  if (study %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
    model_study <- lm_robust(wasted_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                             data = df_study_f)
  } else {
    model_study <- lm_robust(wasted_el ~ iwi + arms_maternal + iwi * arms_maternal, 
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_wasted_f$iwi), by = 1, length.out = 71)
