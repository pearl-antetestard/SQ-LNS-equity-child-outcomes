---
title: "Linear regression - interaction between treatment and IWI - language score, female"
...
inter_f <- df_analysis_language_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_language_f) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(d1_lang_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_language_f, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_language_f$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_f <- df_analysis_language_f %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_language_f) {
    study_name <- unique(df_analysis_language_f$study)
...
point.wise.DF.plot_f = point.wise.DF_f %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled d1_lang_el (female)')) +
  theme_minimal() +
...
study_list <- unique(df_analysis_language_f$study)
results_f <- lapply(study_list, function(study) {
  df_study_f <- df_analysis_language_f %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_language_f$iwi), by = 1, length.out = 71)
