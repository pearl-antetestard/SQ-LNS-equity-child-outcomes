---
title: "Measuring inequalities: RII and SII - anemia, male"
...
# Load the data
df_analysis_anemia_m <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  filter(female == 0) %>%
...
ineq_m <- df_analysis_anemia_m %>%
  ...
  map_dfr(function(df_analysis_anemia_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_anemia_m %>% filter(arms_maternal == "Control")
      ...
      # Intervention group
      df_interv <- df_analysis_anemia_m %>% filter(arms_maternal == "SQ-LNS")
...
prevalence_values_anemia <- df_analysis_anemia_m %>%
...
overall_prevalence_anemia <- df_analysis_anemia_m %>%
  group_by(study) %>%
  summarise(
...
meta_results_rii_m <- rbind(RII_control_df_wide, RII_intervention_df_wide)
...
meta_results_rii_m_prev <- cbind(meta_results_rii_m, meta_results_prevalence)


meta_results_sii_m <- rbind(SII_control_df_wide, SII_intervention_df_wide)
meta_results_sii_m_prev <- cbind(meta_results_sii_m, meta_results_prevalence)

```

```{r plot rii}

meta_rii_anemia_m <-  ggplot(meta_results_rii_m, aes(x = arm, y = RE.meta)) +
...
meta_sii_anemia_m <- ggplot(meta_results_sii_m, aes(x = arm, y = RE.meta*100)) +
  geom_point(aes(color = arm), size = 2.5) +
...
meta_plot_anemia_m <- plot_grid(meta_rii_anemia_m, meta_sii_anemia_m, ncol = 1, nrow = 2)
...
write.csv2(ineq_m, file = here::here("output", "rii_sii_anemia_m.csv"), row.names = FALSE)
