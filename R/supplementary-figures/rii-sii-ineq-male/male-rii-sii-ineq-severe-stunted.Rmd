---
title: "Measuring inequalities: RII and SII - severe stunting, male"
...
ecdf_iwi_decile <- ecdf(df_analysis_severe_stunted_m$iwi_decile)

# Apply the ECDF to get the relative rank
df_analysis_severe_stunted_m <- df_analysis_severe_stunted_m %>%
  mutate(iwi_relative_rank = ecdf_iwi_decile(iwi_decile))

# Print the data frame with the new relative rank column
print(df_analysis_severe_stunted_m)
...
ineq_m <- df_analysis_severe_stunted_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_severe_stunted_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_severe_stunted_m %>% filter(arms_maternal == "Control")
      ...
      # Intervention group
      df_interv <- df_analysis_severe_stunted_m %>% filter(arms_maternal == "SQ-LNS")
...
prevalence_values_severe_stunted <- df_analysis_severe_stunted_m %>%
...
overall_prevalence_severe_stunted <- df_analysis_severe_stunted_m %>%
  group_by(study) %>%
  summarise(
...
meta_rii_severe_stunted_m <- ggplot(aes(x = arm, y = RE.meta), 
                                  data = meta_results_rii_m) +
  geom_point(aes(color = arm), size = 2.5) +
  geom_errorbar(aes(x = arm, ymin = RE.meta.lower, ymax = RE.meta.upper), width = 0.1, data = meta_results_rii_m) +
...
meta_rii_severe_stunted_m

```

```{r plot sii}

meta_sii_severe_stunted_m <- ggplot(meta_results_sii_m, aes(x = arm, y = RE.meta*100)) +
  geom_point(aes(color = arm), size = 2.5) +
...
meta_sii_severe_stunted_m

```

```{r combine}

meta_plot_severe_stunted_m <- plot_grid(meta_rii_severe_stunted_m, meta_sii_severe_stunted_m, ncol = 1, nrow = 2)

meta_plot_severe_stunted_m
```

## Save the results
```{r save results}

write.csv2(ineq_m, file = here::here("output", "rii_sii_severe_stunted_m.csv"), row.names = FALSE)
