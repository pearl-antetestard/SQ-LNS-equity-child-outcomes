---
title: "Measuring ineq_mualities: RII and SII - wasted, male"
...
# Load the data
df_analysis_wasted_m <- readRDS(file = here::here("data", "1-final", "df_analysis_iwi.rds")) %>% 
  filter(female == 0) %>%
...
RII = 1 or SII = 0: no ineq_muality

```{r RII and SII}

ineq_m <- df_analysis_wasted_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wasted_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      # Control group
      df_control <- df_analysis_wasted_m %>% filter(arms_maternal == "Control")
      ...
      # Intervention group
      df_interv <- df_analysis_wasted_m %>% filter(arms_maternal == "SQ-LNS")
...
prevalence_values_wasted <- df_analysis_wasted_m %>%
...
overall_prevalence_wasted <- df_analysis_wasted_m %>%
  group_by(study) %>%
  summarise(
...
meta_rii_wasted_m <- ggplot(aes(x = arm, y = RE.meta), data = meta_results_rii_m) +
...
meta_sii_wasted_m <- ggplot(meta_results_sii_m, aes(x = arm, y = RE.meta*100)) +
   geom_point(aes(color = arm), size = 2.5) +
...
meta_plot_wasted_m <- plot_grid(meta_rii_wasted_m, meta_sii_wasted_m, ncol = 1, nrow = 2)
...
write.csv2(ineq_m, file = here::here("output", "rii_sii_wasted_m.csv"), row.names = FALSE)
