...
inter_m <- df_analysis_wasted_m %>%
  ...
  map_dfr(function(df_analysis_wasted_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(wasted_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_wasted_m, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_wasted_m$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_m <- df_analysis_wasted_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wasted_m) {
    study_name <- unique(df_analysis_wasted_m$study)
...
point.wise.diff.plot_m = point.wise.absolute_diff_m %>%
  ...
  theme(axis.title.y = element_text(size = 18),
      ...
      annotate("text", x = min(point.wise.absolute_diff_m$iwi) * 0.8, 
               y = max(point.wise.absolute_diff_m$RE.meta.upper) + 0.1, 
...
study_list <- unique(df_analysis_wasted_m$study)
results_m <- lapply(study_list, function(study) {
  df_study_m <- df_analysis_wasted_m %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_wasted_m$iwi), by = 1, length.out = 71)
