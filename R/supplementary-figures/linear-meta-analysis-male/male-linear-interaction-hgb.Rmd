---
title: "Linear regression - interaction between treatment and IWI - hemoglobin concentration, male"
...
inter_m <- df_analysis_hgb_m %>%
  ...
  map_dfr(function(df_analysis_hgb_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(b1_hb_el ~ iwi + arms_maternal + iwi * arms_maternal, 
      ...
      model <- lm_robust(b1_hb_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_hgb_m, clusters = cluster)
...
ggplot(inter_m, aes(x = study, y = inter_treat_iwi)) +
  ...
  labs(x = "Study", y = "b1_hb_el (arms_maternalSQ-LNS:iwi)") +
...
forest_meta_m <- forest(meta1_m, 
       layout = "JAMA", 
       leftlabs = c("Study", "b1_hb_el (iwi:arms_maternalSQ-LNS) [95% CI]"),
...
pooled_slope_plot_m <- ggplot(aes(x = term, y = RE.meta), data = laz_eff_df_wide_m) +
  ...
  labs(x = "Random-effect meta-analysis", y = "Pooled b1_hb_el (iwi:arms_maternalSQ-LNS)") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  theme_minimal()  +
...
iwi_values <- seq(min(df_analysis_hgb_m$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_m <- df_analysis_hgb_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_hgb_m) {
    study_name <- unique(df_analysis_hgb_m$study)
...
point.wise.DF.plot_m = point.wise.DF_m %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled b1_hb_el (male)')) +
  theme_minimal() +
...
study_list <- unique(df_analysis_hgb_m$study)
results_m <- lapply(study_list, function(study) {
  df_study_m <- df_analysis_hgb_m %>% filter(study == !!study)
  
  if (study %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
    model_study <- lm_robust(b1_hb_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                             data = df_study_m)
  } else {
    model_study <- lm_robust(b1_hb_el ~ iwi + arms_maternal + iwi * arms_maternal, 
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_hgb_m$iwi), by = 1, length.out = 71)
