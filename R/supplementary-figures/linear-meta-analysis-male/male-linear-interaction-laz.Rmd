---
title: "Linear regression - interaction between treatment and IWI - laz, male"
...
inter_m <- df_analysis_laz_m %>%
  ...
  map_dfr(function(df_analysis_laz_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(laz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_laz_m, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_laz_m$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_m <- df_analysis_laz_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_laz_m) {
    study_name <- unique(df_analysis_laz_m$study)
...
point.wise.DF_m$RE.meta  =        point.wise.DF_m$RE.meta
point.wise.DF_m$RE.meta.upper  =  point.wise.DF_m$RE.meta.upper
point.wise.DF_m$RE.meta.lower  =  point.wise.DF_m$RE.meta.lower
...
study_list <- unique(df_analysis_laz_m$study)
results_m <- lapply(study_list, function(study) {
  df_study_m <- df_analysis_laz_m %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_laz_m$iwi), by = 1, length.out = 71)
...
# Plot
plot_diff_m %>%
  ggplot(aes(x = iwi, y = delta_y_m)) + geom_line(size=0.5)+
