---
title: "Linear regression - interaction between treatment and IWI - severe stunting, male"
...
inter_m <- df_analysis_severe_stunted_m %>%
  ...
  map_dfr(function(df_analysis_severe_stunted_m) {
    ...
    if (study_name %in% c("DOSE")) {
      ...
      model <- lm_robust(severe_stunted ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_severe_stunted_m, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_severe_stunted_m$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_m <- df_analysis_severe_stunted_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_severe_stunted_m) {
    study_name <- unique(df_analysis_severe_stunted_m$study)
...
study_list <- unique(df_analysis_severe_stunted_m$study)
results_m <- lapply(study_list, function(study) {
  df_study_m <- df_analysis_severe_stunted_m %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_severe_stunted_m$iwi), by = 1, length.out = 71)
