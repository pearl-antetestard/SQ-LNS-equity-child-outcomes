---
title: "Linear regression - interaction between treatment and IWI - gross motor score, male"
...
inter_m <- df_analysis_gross_m %>%
  ...
  map_dfr(function(df_analysis_gross_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      ...
      model <- lm_robust(d8_grossmotor_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_gross_m, clusters = cluster)
...
iwi_values <- seq(min(df_analysis_gross_m$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_m <- df_analysis_gross_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_gross_m) {
    study_name <- unique(df_analysis_gross_m$study)
...
point.wise.DF.plot_m = point.wise.DF_m %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled d8_grossmotor_el (male)')) +
  theme_minimal() +
...
study_list <- unique(df_analysis_gross_m$study)
results_m <- lapply(study_list, function(study) {
  df_study_m <- df_analysis_gross_m %>% filter(study == !!study)
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_gross_m$iwi), by = 1, length.out = 71)
