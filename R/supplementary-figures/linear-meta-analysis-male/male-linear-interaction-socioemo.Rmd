---
title: "Linear regression - interaction between treatment and IWI - socioemotional score, male"
...
inter_m <- df_analysis_socioemo_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_socioemo_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(d5_socioemo_el ~ iwi + arms_maternal + iwi * arms_maternal, 
      ...
      model <- lm_robust(d5_socioemo_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_socioemo_m, clusters = cluster)
...
ggplot(inter_m, aes(x = study, y = inter_treat_iwi)) +
  ...
  labs(x = "Study", y = "d5_socioemo_el (arms_maternalSQ-LNS:iwi)") +
...
forest_meta_m <- forest(meta1_m, 
       layout = "JAMA", 
       leftlabs = c("Study", "d5_socioemo_el (iwi:arms_maternalSQ-LNS) [95% CI]"),
...
pooled_slope_plot_m <- ggplot(aes(x = term, y = RE.meta), data = laz_eff_df_wide_m) +
  ...
  labs(x = "Random-effect meta-analysis", y = "Pooled d5_socioemo_el (iwi:arms_maternalSQ-LNS)") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  theme_minimal()  +
...
iwi_values <- seq(min(df_analysis_socioemo_m$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_m <- df_analysis_socioemo_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_socioemo_m) {
    study_name <- unique(df_analysis_socioemo_m$study)
...
point.wise.DF.plot_m = point.wise.DF_m %>%
  ...
  xlab(bquote('IWI')) +
  ylab(bquote('Pooled d5_socioemo_el (male)')) +
  theme_minimal() +
  ...
  theme(axis.title.y = element_text(size = 18),
      ...
      annotate("text", x = min(point.wise.absolute_diff_m$iwi) * 0.8, 
               ...
               label = paste("interaction p-value:", 
                             round(as.numeric(unique(laz_eff_df_wide_m$pval.random)),
                                   3)), 
               hjust = 0, vjust = 1, size = 3, color = "black")
...
study_list <- unique(df_analysis_socioemo_m$study)
results_m <- lapply(study_list, function(study) {
  df_study_m <- df_analysis_socioemo_m %>% filter(study == !!study)
  
  if (study %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
    model_study <- lm_robust(d5_socioemo_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                             data = df_study_m)
  } else {
    model_study <- lm_robust(d5_socioemo_el ~ iwi + arms_maternal + iwi * arms_maternal, 
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_socioemo_m$iwi), by = 1, length.out = 71)
