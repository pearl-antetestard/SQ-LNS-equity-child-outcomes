---
title: "Linear regression - interaction between treatment and IWI - wlz, male"
...
inter_m <- df_analysis_wlz_m %>%
  ...
  map_dfr(function(df_analysis_wlz_m) {
    ...
    if (study_name %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
      model <- lm_robust(wlz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
      ...
      model <- lm_robust(wlz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                         data = df_analysis_wlz_m, clusters = cluster)
...
ggplot(inter_m, aes(x = study, y = inter_treat_iwi)) +
  ...
  labs(x = "Study", y = "wlz_el (arms_maternalSQ-LNS:iwi)") +
...
forest_meta_m <- forest(meta1_m, 
       layout = "JAMA", 
       leftlabs = c("Study", "wlz_el (iwi:arms_maternalSQ-LNS) [95% CI]"),
...
laz_eff_m <- c(meta1_m$TE.random, meta1_m$upper.random, meta1_m$lower.random, 
              meta1_m$seTE.random, meta1_m$Q, meta1_m$df.Q, meta1_m$I2,
              meta1_m$tau2,
              meta1_m$pval.random)
...
pooled_slope_plot_m <- ggplot(aes(x = term, y = RE.meta), 
                              data = laz_eff_df_wide_m) +
  geom_point(size = 3) +
...
iwi_values <- seq(min(df_analysis_wlz_m$iwi), by = 1, length.out = 71)

# Fit the models and generate predictions for both groups with confidence intervals
predictions_m <- df_analysis_wlz_m %>%
  split(.$study) %>%
  map_dfr(function(df_analysis_wlz_m) {
    study_name <- unique(df_analysis_wlz_m$study)
...
plots_m <- predictions_m %>%
  ...
  map(function(data) {
    ggplot(data, aes(x = iwi, y = fit.fit, color = arms_maternal)) +
      geom_line() +
      geom_errorbar(aes(ymin = fit.lwr, ymax = fit.upr, fill = group), 
                    alpha = 0.2) +
...
study_list <- unique(df_analysis_wlz_m$study)
results_m <- lapply(study_list, function(study) {
  df_study_m <- df_analysis_wlz_m %>% filter(study == !!study)
  
  if (study %in% c("DOSE", "GHANA", "HAITI", "DYADG", "DYADM")) {
    model_study <- lm_robust(wlz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
                             data = df_study_m)
  } else {
    model_study <- lm_robust(wlz_el ~ iwi + arms_maternal + iwi * arms_maternal, 
...
# Define IWI values
iwi_vals <- seq(min(df_analysis_wlz_m$iwi), by = 1, length.out = 71)
